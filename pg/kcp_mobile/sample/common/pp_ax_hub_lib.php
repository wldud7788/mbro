<?
  ...
  class   C_PP_CLI
  {
	...
	function mf_exec()
	{
		require_once dirname(__FILE__) . '/../../../../app/libraries/Process.php';

		$path = 'pg/kcp_mobile/log/';
		$arg = func_get_args();
		$rt = '';
		...
		// 실행 명령어 로그
		$file = 'arg_input_' . date('Ymd') . '.log';
		file_put_contents($path . $file, '[' . date('Y-m-d H:i:s') . '] ' . $arg[1] . PHP_EOL, FILE_APPEND);
		$file = 'kcp_lib_cmd_input_' . date('Ymd') . '.log';
		file_put_contents($path . $file, '[' . date('Y-m-d H:i:s') . '] ' . $exec_cmd . PHP_EOL, FILE_APPEND);

		// PG 통신 실행
		$Process = new Process($exec_cmd, null, null, 30);

		try {
			$Process->mustRun();
		} catch (Exception $e) {
			$file = 'kcp_lib_exec_error_' . date('Ymd') . '.log';
			file_put_contents($path . $file, '[' . date("Y-m-d H:i:s") . '] ' . $e->getMessage() . PHP_EOL, FILE_APPEND);
		}

		// 결과 수신 로그
		$rt = $Process->getOutput();
		$file = 'kcp_lib_exec_input_' . date('Ymd') . '.log';
		file_put_contents($path . $file, '[' . date('Y-m-d H:i:s') . '] ' . $rt . PHP_EOL, FILE_APPEND);
