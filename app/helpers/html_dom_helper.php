	...
	function makeup()
	{
		...
		foreach ($this->attr as $key => $val) {
			...
			if ($val === true) {
				...
				// | 값으로 다중 속성값 검사 후 다중 값인경우 여러속성으로 반환 :: 2019-04-24 pjw
				// * html 문법 오류지만 합치거나, 따로 파서 쪽에서 내용을 바꾸면 안되므로 그대로 반환하기 위함
				$dup_attr = explode('$$', $val);
				$dup_cnt  = count($dup_attr);
			
				//  정상의 경우엔 1번 돌고 끝나지만 속성을 중복으로 사용하면 여러번 돈다.
				foreach($dup_attr as $dup_idx => $duplicate_val){
					$ret .= $key
					...
					// 같은 이름의 속성끼리 붙는 걸 방지하기 위해 공백을 줌
					if( $dup_cnt > 1 && $dup_idx < ($dup_cnt - 1) ){
						$ret .= ' ';
					}
				}
			}
...
class simple_html_dom
{
	...
	protected function read_tag()
	{
		...
		do {
			...
			if (strpos($name, '{') !== false){	// 태그속성에 들어있는 Template_ 요소 처리 pjw
				/**
				 * } 뒤에 공백있으면 공백 유지하도록 수정 
				 * 임의로 공백제거 시 html 속성들이 공백없이 붙어서 다른 name 으로 변경되어 수정함
				**/
				$blank_check = $this->pos+1;
				$copy_until = $this->copy_until("}");
				$_blank 	= $this->doc[$blank_check + strlen($copy_until)] == ' ' ? ' ' : '';
				$name		= $name.$copy_until.'}'.$_blank;
			...
			}else if ($name !== '/' && $name !== '') { // this is a attribute name
				// [1] Whitespace after attribute name
				$space[1] = $this->copy_skip($this->token_blank);
	...
	protected function parse_attr($node, $name, &$space)
	{
		...
		// 중복되는 속성은 무시하고 뒤에 붙여서 추후 뽑을 때 array처리로 그대로 나오게끔 처리 2019-04-23 pjw	
		// 뒤에 | 기호로 중복 속성 묶음
		$duplicate = '';		
		if (isset($node->attr[$name]))  $duplicate = $node->attr[$name] . '$$';
		...
		switch ($this->char) {
			case '"': // value is anything between double quotes
				...
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				$node->attr[$name] = $duplicate . $this->restore_noise($this->copy_until_char('"'));
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				break;
			...
			default: // value is anything until the first space or end tag
				$node->_[HDOM_INFO_QUOTE][] = HDOM_QUOTE_NO;
				$node->attr[$name] = $duplicate . $this->restore_noise($this->copy_until($this->token_attr));
		}
