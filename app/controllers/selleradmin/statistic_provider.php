...
class statistic_provider extends selleradmin_base {
	...
	{
		redirect("/selleradmin/statistic_provider/sales_monthly");
	}
	...
	public function sales_monthly()
	{
		...
		$sql = "
			select
				...
				WHERE
				b.provider_seq = '{$_GET['provider_seq']}' )
		...
		for($i=0;$i<$maxMonth;$i++){
			...
			$dataForChart['순매출액'][$i] = $statsData[$i]['sales_price']?$statsData[$i]['sales_price']:0;
		}
	...
	public function sales_daily()
	{
		...
		$sql = "
			select
				...
				ORDER BY
					exp.export_seq desc) as b
				where b.provider_seq = '{$_GET['provider_seq']}'
