...
class Board_comment_process extends admin_base {
	...
	public function index()
	{
		...
		**/
		...
		/* 댓글 삭제 */
		elseif (in_array($mode,['board_comment_delete','report_comment_delete'])) {
			$this->load->library('Board/BoardReportLibrary');

			$delcmtseq = $this->input->post('delcmtseq');

			$parentsql['whereis']	= ' and seq= "'.$delcmtseq.'" ';
			$parentsql['select']		= ' seq, depth ';
			$parentdata = $this->Boardcomment->get_data($parentsql);//댓글정보
			if(empty($parentdata['seq'])) {
				$return = [
			...
			if($replyor==0 || $parentdata['depth'] == 1 ) {//답글이 없거나 답글인 경우 real 삭제
				$result = $this->Boardcomment->data_delete($delcmtseq);
			}else{
				$params['display']			= '1';//삭제글여부1
				$params['subject']			= '';//초기화함
				$params['content']			= '';//초기화함
				$params['r_date']			= date("Y-m-d H:i:s");
				$result = $this->Boardcomment->data_delete_modify($params,$delcmtseq);
			}

			if($result) {

				//댓글평가제거
				$this->Boardscorelog->data_cparent_delete($boarddata['seq'],$delcmtseq);
