...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
	...
	//한글 입력이 늦어지는 문제로 추가한 변수
	var inpKo = false;
    function onKeyDown(e) {
	  if (!focused) onFocus();
      var code = e.keyCode;
	  
	  //변수 초기화 한 후 한글 keyCode가 항상 229로 넘어오므로 229일 때 true
	  inpKo = false;
	  if(code == 229) inpKo = true;
    ...
    var prevInput = "";
    function readInput() {
	  //한글 입력이 되어 있는 경우 return 하지 않는 조건 추가
      if (leaveInputAlone || !focused || (hasSelection(input) && !inpKo)) return false;
	  inpKo = false;
