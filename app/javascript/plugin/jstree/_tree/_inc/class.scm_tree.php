...
class _tree_struct {
	...
	// table method 변경
	function chg_struct_table($table){
		$this->table = $table;
	}
	...
	function _move($id, $ref_id, $position = 0, $is_copy = false) {
		...
		if($is_copy){
			...
			foreach($children as $data)
			{
				...
				$query = "update `".$this->table."` set category_code='$code' where id='".$data['id']."'";
...
class json_tree extends _tree_struct {
	...
	function move_node($data) {
		...
		/* 이동 */
			...
			/* 하위카테고리도 동일한 처리 */
			$this->jstreedb->query("select * from `".$this->table."` where category_code like '{$ori_category_code}%' and category_code!='{$ori_category_code}'");
