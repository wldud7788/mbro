...
class _database {
	...
	function connect() {
		if (!$this->link) {
			$this->link = mysqli_connect($this->settings["servername"].":".$this->settings["serverport"],	$this->settings["username"], $this->settings["password"], $this->settings["database"]);
		}
		if( !$this->link ) {
			return false;
		}
	...
	function nextr() {
		...
		unset($this->row);
		$this->row = mysqli_fetch_array($this->result, MYSQL_BOTH);
