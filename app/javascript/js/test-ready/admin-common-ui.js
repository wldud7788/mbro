...
function addSelectDateEvent()
{
	...
	$(".select_date").on("click", function()
	{
		if($(this).parent('div').length>0) {
			$(this).parent('div').find('.select_date').removeClass("on");
			$(this).parent('div').find('.select_date').css("border", "1px solid #dcdcde");
		} else {
			$(".select_date").removeClass("on");
			$(".select_date").css("border", "1px solid #dcdcde")
		}
		$(this).addClass("on");
		$(this).css("border", "1px solid #3090d6");
		$(this).next('.select_date').css("border-left", "1px solid #3090d6");
	})
}
...
function setRangeContentsSelect(name, value, obj, is_array)
{
	if( typeof is_array == "undefined" || is_array ==  "" || is_array === null ){
		var inputs = $('select:[name='+ name +']');
	} else {
		var inputs = $('select:[name^='+ name +']'); 
	}
	
	if(obj) inputs = obj.find('select:[name='+ name +']');
	if (inputs.length == 0 ) return;

	inputs.on('change', function(){

		var len= $(this).find("option").length;
		var target = $(this).closest("td").find("."+ name +"_" + $(this).val())

		for(var i=0; i<len; i++)
		{
			var tg = $(this).closest("td").find("."+ name +"_" + $(this).find("option").eq(i).val())
			tg.hide();
		}
...
function tooltipSetPos(obj, btn, size)
{
	...
	if(winH - (posY - scrollTop) - tgH < 0)
	{
		if( winW - posX - tgW < 0)
		{
			...
			obj.css("top", posY - (tgH - (winH - (posY - scrollTop))));
		}
