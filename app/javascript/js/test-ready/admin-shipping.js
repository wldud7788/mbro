...
function ship_grp_sel(type, provider_seq){
	...
	if((displayTerms == 'AUTO' && displayTermsType == 'LAYAWAY')){
		...
		var msg				= "<div class='left pdb10'><span class='bold'>[예약상품 배송그룹 연결 권장사항]</span><br><br>본 상품은 예약기간 종료 후 배송하는 예약상품으로 본 상품만 배송하기 위한 배송그룹을 연결하세요.<br>즉, 해당 배송그룹에 연결된 상품은 본 상품 1개만 있어야 합니다.<br><br>예시1) ‘A’ 배송그룹에 1개의 예약상품을 연결하였을 경우<br>└ <font color='red'>문제가 없습니다. 권장합니다.</font><br><br>예시2) ‘A’ 배송그룹에 2개의 예약상품을 연결하였을 경우<br>2개의 예약상품이 함께 주문되었고<br>2개의 예약상품에 예약발송일이 다를 경우 가장 늦은 예약발송일이 소비자에게 안내됩니다.<br><br>예시3) 다른 일반상품이 연결된 ‘A’ 배송그룹에 예약상품을 연결하였을 경우<br>일반상품과 예약상품이 함께 주문되면 예약발송일이 소비자에게 안내되며, <br>관리자는 일반상품 부분출고, 예약상품 입고 후 예약상품 부분출고하거나<br>또는 예약상품 입고 후 일반상품과 예약상품을 함께 출고처리 할 수 있습니다.<br></div>";
...
function shipping_address_pop(use_type){
	...
	var params = {'use_type':use_type}; // 팝업창 사용처
...
function ship_chg_popup(identity_seq, p_type, process){
	...
	openDialog("배송그룹/방법 변경", "shipping_chg_pop_area", {"width":370,"height":250});
}
...
function address_pop(address_category, address_name, view_address, shipping_phone){
	...
	openDialog("출고지 안내", "address_dialog", {"width":430,"height":280});
}
...
function set_lang_pop(type){
	if	(type == 'view'){
		openDialog("리스트페이지의 배송안내 문구 설정", "ship_txt_view_lay", {"width":"900","height":"390","show" : "fade","hide" : "fade"});
	}else{
		openDialog("리스트페이지의 배송안내 문구 설정", "ship_txt_set_lay", {"width":"900","height":"400","show" : "fade","hide" : "fade"});
	}
}
