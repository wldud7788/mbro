$(document).ready(function() {
	...
	$('#page-global-btn').css('top', 67);
	$('.page-manual-btn').css('top', 67);

	$('#page-title-bar').on('classChange', function(){
		if ($(this).hasClass('flyingMode')) {
			$('#page-global-btn').css('top', 77);
			$('.page-manual-btn').css('top', 77);
		} else {
			$('#page-global-btn').css('top', 67);
			$('.page-manual-btn').css('top', 67);
		}
	...
	$("input[name='chkall']").on("click",function(){
		...
		if($(this).val() == "goods"){
			selector = $(this).closest("div").next(".goods_list").find(".chk");
		}
	...
	if(typeof gift == "undefined" && batchModify === false && window.Firstmall.Config.Environment.serviceLimit.H_AD == true){
		...
		if(gl_provider_seq > 1){
			do_provider_change(gl_provider_seq);			// 입점사 선택 시 기본 정보 세팅(입점사명, 정산방법, 배송방법 등)
	...
	$("#goodsImageTable").on("click", ".batchImageMultiRegist, .batchImageRegist, .eachImageRegist", function(){
		...
		if(_type == "multi"){
			...
			if(params.division == 'view') height = 540;
			// 순번 사진(대표사진 외 6장)
		}else{
	...
	$(".batchVideoRegist").on("click",function(){
		...
		window.open('video_upload?no='+gl_goods_seq+'&uptype='+uptype,'VideoPopup','width=550,height=350');
	});
	...
	/* 상품코드 코드생성넣기*/
	$("#optionLayer").on("click", "#goodsCodeBtn", function(){openDialog("기본코드 자동생성", "makeGoodsCodLay", {"width":"400","height":"230"});});
	...
	$(".btn_goods_default_set").on("click", function(){
		var _type 		= $(this).attr("data-type");
		var _title 		= "";
		var _options 	= {'width':420,'height':280,'show':'fade','hide' : 'fade'};
		if(_type == "option"){
			_title = "옵션 보기 기본 설정";
		...
		$.ajax({
			...
			data: "goods_kind=goods&sub_kind="+_type,
			success: function(html){
	...
	$("#memberInputMake").on("click", function(){
		if	(!$(this).closest('table').find("input[name='memberInputUse']:checked").val() == "1"){
			...
			openDialogConfirm('['+goods_name+'] 상품의 <br/>추가입력옵션 정보를 가져오시겠습니까?',400,200,function(){
				...
				openDialog("추가 입력 옵션 생성/수정", "memberInputDialog", {"width":"750","height":"500","show" : "fade","hide" : "fade"});
			});
			//}
		}else{
			openDialog("추가 입력 옵션 생성/수정", "memberInputDialog", {"width":"750","height":"500","show" : "fade","hide" : "fade"});
	...
	/* 구매자 추가입력 한줄 추가 */
	$("button#addMemberInputMake").on("click",function(){
	...
	*/
	...
	$('button#popStringPriceBtn').on("click",function(){
		...
		check_string_price();
		openDialog("구매 가능 대상 설정", "popStringPrice", {"width":"750","height":"660","show" : "fade","hide" : "fade"});
	});
	...
	*/
	...
	$("#coupon_serial_upload").on("click", function(){
		...
		if ($("input[name='coupon_serial_type'][value='n']").is(":checked"))
			openDialog("엑셀 등록", "coupon_serial_upload_lay", {"width":430,"height":330});
	...
	*/
	...
	if (batchModify === false && socialcpuse_flag) {
		...
		$("button#coupon_group_search").on("click", function() {
			...
			if(provider_seq){
				...
				addFormDialog('./social_goods_group?type=write&sel_group_seq='+group_seq+'&provider_seq='+provider_seq, '700', '450', '['+provider_name+']티켓상품그룹 찾기 ','false','resp_btn v3 size_XL');
			}else{
	...
	if (batchModify === false && gl_goods_seq) {
		$("form[name='goodsRegist'] input[name='tmp_goodsView'][value='"+goodsObj.goods_view+"']").prop("checked",true);
		setGoodsView(goodsObj.goods_view);
	}
	...
	$('#subOptionProcessBtn').on("click", function(){
		if(window.Firstmall.Config.Environment.serviceLimit.H_FR == true){
			...
			openDialog("추가 구성 옵션 구매 방법 설정", "subOptionProcessSet", {"width":"700", "height":"300","show" : "fade","hide" : "fade"});
		}
	});
	...
	/*
	$("select[name='hscode_selector']").css({'width': '150px'}).combobox()
		.change(function(){
		...
		.bind('focus',function(){
			if($(this).val()==$( "select[name='hscode_selector'] option:first-child" ).text()){
				$(this).val('');
			}
		})
		.bind('mouseup',function(){
			if($(this).val()==''){
				$( "select[name='hscode_selector']").next(".ui-combobox").children("a.ui-combobox-toggle").click();
			}
	});
	*/
	...
	$(".runout_setting").on("click",function(){
		if(window.Firstmall.Config.Environment.serviceLimit.H_FR == true){
			...
			if($("input[name='runout_type']:checked").val() == "goods"){
				openDialog("재고에 따른 판매 - 개별 설정", "popup_runout_setting", {"width":800,"height":"260","show" : "fade","hide" : "fade"});
			}else{
	...
	$("#possible_pay_setting").on("click",function(){
		if(window.Firstmall.Config.Environment.serviceLimit.H_FR == true){
			...
			openDialog("결제 수단 개별 설정", "possible_pay", {"width":"500", "height":"390","show" : "fade","hide" : "fade"});
		}
	});
