...
require_once dirname(__FILE__).'/Message/Hash/SHA256.php';
require_once dirname(__FILE__).'/Message/Message.php';

//include_once dirname(__FILE__)."/PEADR.php";
...
class NHNAPISCL
{
	...
	function generateSign($data, $key){
	   ...
	   $crypt = Message::createHMAC('SHA256', $key);
		//debug($crypt);
		// $crypt	= base64_encode(hash_hmac('sha256',$data,$key));

	   //$crypt	= hash_hmac('SHA256',$data,$key,true);
