...
class Template_
{
	...
	function print_($fid, $scope='', $sub=0)
	{
		...
		$this->safe_mode = true;
		if ($this->phpSkin['able'] != 'N') { // 특정상점 php 코드 사용 허용
			$this->safe_mode = false;
		}

		foreach ($this->safe_mode_except as $file) {
			if (preg_match('/' . $file . '/', $tpl[1])) {
				$this->safe_mode = false;
			}
		}

		// 관리자 입점사 관리자 php 코드 허용
		if (preg_match('/admin\/|selleradmin\//', $this->template_dir)) {
			$this->safe_mode = false;
		}
		...
		if($fid == 'LAYOUT_FOOTER'){
			$this->CI =& get_instance();

            // ga4 연동
            $this->CI->load->library('ga4library');
            $this->ga_html = $this->CI->ga4library->init($this->ga_html);
			
            // ifdo 연동
			$this->CI->load->library('ifdolibrary');
			$this->ga_html = $this->CI->ifdolibrary->init($this->ga_html);

			// 채널톡 연동
			$this->CI->load->library('channeltalklibrary');
			$this->ga_html = $this->CI->channeltalklibrary->init($this->ga_html);
		}

		if	($fid == 'LAYOUT_FOOTER' && $this->ga_html){
			echo $this->ga_html;
		}
	...
	function _get_compile_path($rpath)
	{
		...
		if( isInstalledRunkit() ){
			...
			$customLoadingFile = APPPATH."../custom/".__CUSTOM_CONFIG_SITEID__.DIRECTORY_SEPARATOR.str_replace(str_replace("system/","",BASEPATH),"",$this->template_dir).DIRECTORY_SEPARATOR.($this->skin?$this->skin.'/':'').$rpath;
			if (is_file($customLoadingFile)) $tpl_path = $customLoadingFile;
		}
		...
		if (@!is_file($tpl_path)) {
			...
			if( !$_GET['createCached'] && !$_POST['createCached'] ) $this->exit_('Error #1',$errmsg);
		}
