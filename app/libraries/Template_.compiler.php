...
class Template_Compiler_
{
	 ...
	 */
		if ($this->safe_mode) {
			require_once (APPPATH . 'helpers/antimalware_helper' . EXT);
			$patternFound = scanFile($tpl_path);
			if (count($patternFound) > 0) {
				$aErr = end($patternFound);
				echo '<span style="font:12px tahoma,arial;color:#0071DC;background:white">Error</span>';
				exit;
			}
		}
	...
	// disable php tag
		...
		switch ($this->compileSkinType()) {
			case 'admin' : // no break
			case 'admincrm' :  // no break
			case 'selleradmin' : // no break 
			...
			case 'custom' : // no break
			case 'client' :
				/*
				* 2013-04-11 스킨에서 특정함수 차단
				* 2022-02-16 스킨단에서 include, require 직접 사용 금지
				*/
				$safe_pattern = [
					'/(system|passthru|shell_exec|exec)\s*\((.*)\)/i',
					'/(?<!(\"|\'|\_|\-))(include_once|include|require_once|require)(?!(\_|\-))/mi',
				];
				$safe_replacement = [
					'"SERVER-SIDE-SCRIPT-DISABLED"',
					'$1 //$2$3$4$5',
				];
				$source = preg_replace($safe_pattern, $safe_replacement, $source);

				/**
				 * 2022-03-03 requirejs, .required에 걸린 공백 및 주석 제거
				 */
				$safe_pattern = [
					'/(?<!(\"|\'|\_|\-))(\s)(\/\/)(requirejs)(?!(\_|\-))/mi',
					'/(?<!(\"|\'|\_|\-))(\s)(\/\/)(required)(?!(\_|\-))/mi',
	...
	// <!--, --> html 주석 추가 (아이디자인 텍스트 수정기능) :: 2019-03-04 pjw
		$nl_cnt=1;
		$nl_del_sum=0;
		$this->nl_del[0]=0;
		...
		for ($j=0,$i=0,$s=count($split); $i<$s; $i++) {
			...
			switch ($split[$i]) {
			case'<!--':
				$split[$i+1] = preg_replace('/designElement\=(\'|\")text(\'|\")/i', 'designElementTmpText', $split[$i+1]);
				break;
			case'-->':
				// 아무 작업 하지않으나 있어야함
				break;
		...
		############################### 아이디자인 텍스트 수정 기능 ######################################
		// 아이디자인 용 텍스트 수정 textTemplatePath 속성 치환 추가 :: 2019-01-31 pjw
		// 주석 안에 있는 속성은 임시로 다른 값으로 치환 후 인덱스를 매기고 다시 원래속성으로 변환 :: 2019-03-04 pjw

		// 인덱스 번호 부여
		$tpl_path		= str_replace($tpl->template_dir.'/', '', $tpl_path);
		$relative_path	= base64_encode($tpl_path);
		$source			= preg_replace('/[^\[]designElement\=(\'|\")text(\'|\")/i', ' designElement="text" textTemplatePath="'.$relative_path.'" ', $source);
		$source			= preg_replace_callback('/[^\[]designElement\=(\'|\")text(\'|\")/i', function($matches){
				static $id = 0;
				$id++;
				return ' designElement="text" textIndex="'.$id.'" ';
		}, $source);

		// 주석 안의 임시속성 원래 속성으로 변경
		$source			= preg_replace('/designElementTmpText/i', 'designElement="text"', $source);
		############################### 아이디자인 텍스트 수정 기능 ######################################
				...
				case '(' :
					if ($var_state[0]) {
						...
						if (in_array($func, $this->func_plugins)) {
							...
							// safe_mode function 체크 기능 제외(추가 함수 사용처 다양)
							/*
	...
	public function compileSkinType()
	{
		$skinType = 'client';
		...
		// 관리자 crm 스킨
		} elseif (preg_match('/^admincrm\//', uri_string())) {
			$skinType = 'admincrm';
		
		// 관리자 입점사 스킨
		} elseif (preg_match('/^selleradmin\//', uri_string())) {
			$skinType = 'selleradmin';
