...
class Message {/*{{{*/
		...
		if (!function_exists('hash_hmac')) {
			if (!Message::_inFallback($hash_name)) {
				...
				if (in_array($hash_name, $cannot_hmac)) {
					...
					include_once dirname(__FILE__)."/HMAC/{$hash_name}_Fallback.php";
			...
			if (in_array($hash_name, $cannot_hmac)) {
				...
				include_once dirname(__FILE__)."/HMAC/{$hash_name}.php";
				$hmac = "Message_HMAC_{$hash_name}";
				$aa = new $hmac($key, $ser, $enc);
				return $aa;
			}
		}
	}/*}}}*/
	...
	function _mangle($hash_name) {/*{{{*/
		...
		/*
		if (preg_match('/^HASH_HMAC_/', $hash_name)) {
			$hash = $hash_name;
		} else {
			$hash = 'HASH_HMAC_'.$hash_name;
		}
		*/
	...
	function _inFallback($hash_name) {
		return in_array(strtolower($hash_name), array('md5', 'sha1','sha256'));
	}
