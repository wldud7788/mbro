...
class JCryption
{
	...
	public function getPublicKey()
	{
		...
		echo (($_GET['callback'])?$_GET['callback']."(":"").json_encode(array('publickey' => file_get_contents($this->public_key_file))).(($_GET['callback'])?");":"");
		exit();
	}
	...
	{
		$postkey = $_POST['key'];
		if( $_GET['key']){$postkey = $_GET['key'];}
		openssl_private_decrypt(base64_decode($postkey), $key, file_get_contents($this->private_key_file));
		$_SESSION[self::SESSION_KEY] = $key;
		Header('Content-type: application/json');
		echo (($_GET['callback'])?$_GET['callback']."(":"").json_encode(array('challenge' =>  sqAES::crypt($key, $key))).(($_GET['callback'])?");":"");
		exit();
	}
	...
	public static function decrypt_key($sKey, $sEnc)
	{	
		parse_str(sqAES::decrypt($sKey, $sEnc), $output);
		return $output;
	}
