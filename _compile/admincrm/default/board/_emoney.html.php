<?php /* Template_ 2.2.6 2022/05/17 12:05:21 /www/music_brother_firstmall_kr/admincrm/skin/default/board/_emoney.html 000013563 */ ?>
...
$(document).ready(function() {
	...
	$("button.board_emoneyt_btn").live("click",function(){
		...
		openDialog(" ["+managername+"]   <span class='desc'>마일리지지급</span>", "BoardEmoneytlayList", {"width":"550","height":"380","show" : "fade","hide" : "fade"});
	});
	...
	$("button.review_emoneyt_btn").live("click",function(){
		...
		openDialog(" ["+managername+"]   <span class='desc'>마일리지지급</span>", "ReviewEmoneytlayList", {"width":"550","height":"380","show" : "fade","hide" : "fade"});
	});
...
<input type="hidden" name="mbtel" id="emoney_review_mbtel" value="" />
	<table class="info-table-style" style="width:100%">
		...
		<tr>
			...
			<td class="its-td">
				<input type="text" name="goods_review_emoney"  id="goods_review_emoney"  size="5" value="<?php if($TPL_VAR["reserve_goods_review"]){?><?php echo trim($TPL_VAR["reserve_goods_review"])?><?php }else{?><?php echo trim($TPL_VAR["goods_review_emoney"])?><?php }?>" title="" class="line onlyfloat" />
				<a href="/admin/board/manager_write?id=goods_review" ><span class="highlight-link hand">[마일리지 설정하기]</span></a>
			</td>
		</tr>

		<tr>
			<th class="its-th-align center" >유효기간</th>
			<td class="its-td"> <select name="goods_review_reserve_select">
...
<?php }}?>
		...
		<tr>
			<th class="its-th-align center" >SMS <input type="checkbox" name="board_sms"  value="1" id="board_review_sms"  <?php if($TPL_VAR["isdemo"]["isdemo"]){?> <?php echo $TPL_VAR["isdemo"]["isdemojs1"]?> <?php }?>  /></th>
			<td class="its-td">
				<div>
					<ul>
...
<input type="hidden" name="mbtel" id="board_emoney_mbtel" value="" />
	<table class="info-table-style" style="width:100%">
		...
		<tr>
			<th class="its-th-align center" >지급액</th>
			<td class="its-td">
				<input type="text" name="board_emoney"  id="board_emoney"  size="3" value="<?php if($TPL_VAR["reserve_goods_review"]){?><?php echo trim($TPL_VAR["reserve_goods_Board"])?><?php }else{?><?php echo trim($TPL_VAR["board_emoney"])?><?php }?>" title="" class="line onlyfloat" />
			</td>
		</tr>

		<tr>
			<th class="its-th-align center" >유효기간</th>
			<td class="its-td"> <select name="board_reserve_select">
					<option value="" selected >제한하지 않음</option>
...
<?php }}?>
		...
		<tr>
			<th class="its-th-align center" >SMS <input type="checkbox" name="board_sms" id="board_sms"  value="1"  <?php if($TPL_VAR["isdemo"]["isdemo"]){?> <?php echo $TPL_VAR["isdemo"]["isdemojs1"]?> <?php }?>  /></th>
			<td class="its-td">
				<div>
					<ul>
