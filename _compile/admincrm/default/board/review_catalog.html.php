<?php /* Template_ 2.2.6 2022/05/17 12:05:21 /www/music_brother_firstmall_kr/admincrm/skin/default/board/review_catalog.html 000006792 */ 
...
<div class="search-form-container" align="center">
	<table class="search-form-table">
		...
		<tbody class="ltb otb">
			...
			<tr class="list-row">
				<td class="its-td">
					<input type="text" name="search_text" size="35" value="<?php echo $_GET["search_text"]?>" title="상품명, 제목, 내용">
					<label class="pdl10 pdr10"><input type="checkbox" name="gb[]" value="buyed" <?php if($_GET["gb"]&&in_array('buyed',$_GET["gb"])){?>checked<?php }?>/> 구매한 후기</label>
					<label class="pdr10"><input type="checkbox" name="gb[]" value="unbuyed" <?php if($_GET["gb"]&&in_array('unbuyed',$_GET["gb"])){?>checked<?php }?>/> 미구매 후기</label>
					<label><input type="checkbox" name="gb[]" value="best" <?php if($_GET["gb"]&&in_array('best',$_GET["gb"])){?>checked<?php }?>/> 베스트 후기</label>
				</td>
			</tr>
			<tr>
...
<?php if($TPL_record_1){foreach($TPL_VAR["record"] as $TPL_V1){?>
		<tr class="list-row">
			...
			<td align="center"><?php if($TPL_V1["best"]=="checked"){?><span class="red">★</span><?php }else{?><span class="gray">★</span><?php }?></td>
			<td align="left"><span class="boad_view_btn hand" board_id="goods_review" board_seq="<?php echo $TPL_V1["seq"]?>" viewlink="/admin/board/goods_review_view?id=goods_review&seq=<?php echo $TPL_V1["seq"]?>&callPage=crm"><?php echo $TPL_V1["subject"]?></span></td>
			<td align="center"><?php echo $TPL_V1["scorelay"]?></td>
			<td align="center"><?php echo $TPL_V1["r_date"]?></td>
			<td align="center"><?php echo $TPL_V1["hit"]?></td>
			<td align="center"><?php echo $TPL_V1["buyertitle"]?></td>
			<td align="center"><?php echo $TPL_V1["autoemoneylay"]?></td>
...
<?php }else{?>
		<tr class="list-row">
			<td align="center" colspan="11">등록된 상품후기가 없습니다.</td>
		</tr>
<?php }?>
	</tbody>
</table>
