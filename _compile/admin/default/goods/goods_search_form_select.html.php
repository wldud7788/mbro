<?php /* Template_ 2.2.6 2022/05/17 12:31:50 /www/music_brother_firstmall_kr/admin/skin/default/goods/goods_search_form_select.html 000021543 */ 
...
<?php if(serviceLimit('H_AD')){?>
			<!-- 입점사 -->
			<tr <?php if($TPL_VAR["shipping_type"]){?>class="hide"<?php }?>>
				<th class="its-th-align"><span>입점사</span></th>
				<td class="its-td" colspan="5">
					<span>
						<select name="provider_seq_selector" class="line">
<?php if($TPL_VAR["provider_fix"]){?>
						<option value="<?php echo $_GET["provider_seq"]?>" selected ><?php echo $_GET["provider_name"]?>(<?php echo $_GET["provider_id"]?>)</option>
<?php }else{?>
...
<?php }?>
			...
			<tr>
				...
				<td class="its-td" <?php if($TPL_VAR["package"]){?>colspan="3"<?php }?>>
					<span>
...
<?php }?>
			...
			<tr>
				...
				<td class="its-td" colspan="5">
					<div id="src_keyword_lay">
						<input type="text" id="selectKeyword" name="selectKeyword" class="src-keyword-input" value="<?php echo $_GET["selectKeyword"]?>"  title="상품명,상품고유값,상품코드,태그,간략설명" style="padding:0px;height:30px;width:85%;" />
						&nbsp;&nbsp;
						<span class="btn large cyanblue"><button type="submit" style="width:80px;">검색</button></span>
					</div>
					<!-- 검색어 입력시 레이어 박스 : end -->
				</td>
			</tr>
