<?php /* Template_ 2.2.6 2021/08/25 16:14:42 /www/music_brother_firstmall_kr/admin/skin/default/goods/gift_regist.html 000093961 */ 
...
<style type="text/css">
	#page-title-bar-area #page-title-bar .page-global-btn, #page-title-bar-area #page-title-bar .page-manual-btn {top:14px !important;}
	#page-title-bar-area #page-title-bar.flyingMode .page-global-btn, #page-title-bar-area #page-title-bar.flyingMode .page-manual-btn {top:14px !important;}
</style>
...
<div id="page-title-bar-area">
	<div id="page-title-bar">
		...
		<ul class="page-buttons-left">
			<li><span class="btn large white"><button type="button" onclick="document.location.href='/admin/goods/gift_catalog?<?php echo $TPL_VAR["query_string"]?>';">사은품리스트<span class="arrowright"></span></button></span></li>
			<!-- <?php if($TPL_VAR["goods"]["goods_seq"]){?> -->
		...
		<div class="page-goods-helper-btn" >
			<table border="0" cellpadding="0" cellspacing="0" width="100%">
				<tr>
					<td class="ctab"><a href="#01">상품명/검색어 <img src="/admin/skin/default/images/common/btn_quick.gif" align="absmiddle"></a></td>
					<td class="ctab"><a href="#02">판매정보 <img src="/admin/skin/default/images/common/btn_quick.gif" align="absmiddle"></a></td>
					<td class="ctab"><a href="#04">사진 <img src="/admin/skin/default/images/common/btn_quick.gif" align="absmiddle"></a></td>
					<td class="ctab"><a href="#05">관리메모 <img src="/admin/skin/default/images/common/btn_quick.gif" align="absmiddle"></a></td>
				</tr>
			</table>
		</div>
		<!-- 상단 단계 링크 : 끝 -->
...
<!-- 서브메뉴 바디 : 시작-->
<div class="clearbox" style="height:28px;"></div>
<!-- 본사매입 상품 -->
<?php if(serviceLimit('H_AD')){?>
<?php if($TPL_VAR["provider_seq"]=='1'){?>
	<!-- 본사매입 상품 -->
	<div class="providerTitleContainer clearbox bg-blue">
		<div class="ptc-title" style="float:inherit;">본사 매입 상품<?php if($TPL_VAR["goods"]["provider_status_text"]){?> → <?php }?> <?php echo $TPL_VAR["goods"]["provider_status_text"]?></div>
	</div>
	<input type="hidden" class="provider_seq" name="provider_seq" value="1" />
<?php }elseif($TPL_VAR["goods"]["goods_seq"]){?>
	<!-- 입점사 상품 수정 -->
	<div class="providerTitleContainer clearbox bg-red">
		<div class="ptc-title">입점사 상품 - <?php echo $TPL_VAR["provider"]["provider_name"]?> </div>
		<div class="ptc-desc">
			<div class="ptc-charges">
<?php if($TPL_options_1){foreach($TPL_VAR["options"] as $TPL_V1){?>
<?php if($TPL_V1["default_option"]=='y'){?>
<?php if($TPL_VAR["provider_charge"][ 0]["commission_type"]=='SACO'||$TPL_VAR["provider_charge"][ 0]["commission_type"]==''){?>수수료 방식<?php }else{?>공급가 방식<?php }?>
				<?php echo $TPL_V1["commission_rate"]?>

<?php if($TPL_V1["commission_type"]=='SUPR'){?>원<?php }else{?>%<?php }?>
				(옵션이 있을 경우 대표 옵션 기준)
<?php }?>
<?php }}?>
			</div>
...
<?php }else{?>
			<div class="ui-widget">
				<select name="provider_seq_selector" id="providerSeqSelector" style="vertical-align:middle;">
				<option value="">= 선택 =</option>
<?php if($TPL_provider_list_1){foreach($TPL_VAR["provider_list"] as $TPL_V1){?><option value="<?php echo $TPL_V1["provider_seq"]?>" provider_status="<?php echo $TPL_V1["provider_status"]?>" <?php if($TPL_V1["provider_status"]=='N'){?> disabled="1"<?php }?>><?php echo $TPL_V1["provider_name"]?>(<?php echo $TPL_V1["provider_id"]?>)</option><?php }}?>
				</select>
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				<input type="hidden" class="provider_seq" name="provider_seq" value="" />
				<input type="text" name="provider_name" value="" readonly />
			</div>
...
<div class="clearbox" class="mt20"></div>
...
<?php }else{?>
				<span class="btn small"><button type="button" class="runout_setting">설정</button></span>
<?php }?>
			</th>
...
<div class="item-title">
	수출입상품코드 (HS CODE)
	<span class="btn small"><button type="button" id="hscode_detail">자세히</button></span>
		<span class="btn small"><button type="button"  id="hscode_set">HS CODE 설정</button></span>
</div>
<table class="info-table-style" style="width:100%">
<tr>
...
<table class="info-table-style" style="width:100%">
	...
	<thead>
		<tr>
			...
			<th class="its-th-align center">캐시/포인트를 교환하면 → 사은품 지급(배송)</th>
		</tr>
	</thead>
	<tbody>
		<tr>
			...
			<td class="its-td">
				...
				<span style="color:#676767;">※ 캐시/포인트 교환 신청은 MY페이지에서 가능합니다.</span>
			</td>
		</tr>
	</tbody>
</table>
...
<?php if(!$TPL_VAR["images"]){?>
				<div id="multiadd" class="hide">
					<span class="btn large"><button type="button" class="batchImageMultiRegist" style="width:120px;">여러 컷 일괄등록</button></span>
...
<?php if($TPL_I1== 0){?>
			<td class="its-td-align left firstCol pdl30" rowspan="<?php echo count($TPL_VAR["images"])?>">
				...
				<div class="pdt10"><span class="btn large"><button type="button" class="ImageSort" style="width:120px;">순서변경 및 삭제</button></span></div>
			</td>
<?php }?>
