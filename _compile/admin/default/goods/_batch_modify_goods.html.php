<?php /* Template_ 2.2.6 2022/05/17 12:31:55 /www/music_brother_firstmall_kr/admin/skin/default/goods/_batch_modify_goods.html 000011392 */ 
...
<?php }}?>
		...
		<th>
			<input type='text' name="all_summary" class="all_summary_value" value="" apply_target="summary" style="width:70%;"><span class="btn small gray ml3"><button type="button" class="applyAllBtn" apply_type="all_summary">▼</button></span>
		</th>
	</tr>
	</thead>
</table>
...
<?php if(is_array($TPL_R2=$TPL_VAR["arr_common"]['colorPickList'])&&!empty($TPL_R2)){foreach($TPL_R2 as $TPL_V2){?>
					<label style="background-color:#<?php echo $TPL_V2["code"]?>;margin:1px;" class="<?php if(in_array($TPL_V2["code"],$TPL_V1["color_pick_list"])){?>active<?php }?>" alt="<?php echo $TPL_V1["name"]?>" title="<?php echo $TPL_V1["name"]?>"><input type="checkbox" name="color_pick[<?php echo $TPL_V1["goods_seq"]?>][]" value="<?php echo $TPL_V2["code"]?>" <?php if(in_array($TPL_V2["code"],$TPL_V1["color_pick_list"])){?>checked<?php }?> class="color_pick" apply_type="color_pick<?php echo $TPL_V1["goods_seq"]?>" /></label>
<?php }}?>
				</div>
			</td>
