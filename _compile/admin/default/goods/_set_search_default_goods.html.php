...
<?php }}?>
				...
				<div style="padding-left:5px;">
						<label><input type="radio" name="search_provider_status" class="dig_search_provider_status" value="" <?php if($_GET["search_provider_status"]==''){?>checked<?php }?> /> 전체</label>
						<label><input type="radio" name="search_provider_status" class="dig_search_provider_status" value="2" <?php if($_GET["search_provider_status"]== 2){?>checked<?php }?> /> 승인</label>
						<label><input type="radio" name="search_provider_status" class="dig_search_provider_status" value="1" <?php if($_GET["search_provider_status"]== 1){?>checked<?php }?> /> 미승인</label>
					</div>
					<div style="padding:5px 0 0 5px;">
						(<label class="ft_11"><input type="checkbox" name="provider_status_reason_type[]" class="dig_provider_status_reason_type" value="1" <?php if(in_array('1',$_GET['provider_status_reason_type'])){?>checked<?php }?> title="[자동] 최초 상품등록 후 아직 승인되지 않은 미승인 상품"/> 최초등록</label>
						<label class="ft_11"><input type="checkbox" name="provider_status_reason_type[]" class="dig_provider_status_reason_type" value="3" <?php if(in_array('3',$_GET['provider_status_reason_type'])){?>checked<?php }?> title="[자동] 승인된 상품의 정보가 수정되어 미승인된 상품"/> 정보수정</label>
						<label class="ft_11"><input type="checkbox" name="provider_status_reason_type[]" class="dig_provider_status_reason_type" value="2" <?php if(in_array('2',$_GET['provider_status_reason_type'])){?>checked<?php }?>/> 관리자</label>
						<label class="ft_11"><input type="checkbox" name="provider_status_reason_type[]" class="dig_provider_status_reason_type" value="e" <?php if(in_array('e',$_GET['provider_status_reason_type'])){?>checked<?php }?> title="[수동] 승인된 상품을 관리자(통신판매중계자)가 미승인한 상품"/> 기타</label>)
					</div>
					<script type="text/javascript">
					<!--
					$(function(){
				...
				<td class="its-td">
					<select name="commission_type_sel" style="width:72px">
						...
						<option value="SACO" <?php if($_GET["commission_type_sel"]=='SACO'){?>selected<?php }?>>수수료율(%)</option>
						<option value="SUCO" <?php if($_GET["commission_type_sel"]=='SUCO'){?>selected<?php }?>>공급율(%)</option>
						<option value="SUPR" <?php if($_GET["commission_type_sel"]=='SUPR'){?>selected<?php }?>>공급가</option>
					</select>
...
<?php }?>
				...
				<td class="its-td" colspan="3">
					<label><input type="checkbox" name="feed_status" value="Y" <?php if($_GET["feed_status"]=='Y'){?>checked<?php }?> /> 전달</label>
					<label><input type="checkbox" name="feed_status" value="N" <?php if($_GET["feed_status"]=='N'){?>checked<?php }?> /> 미전달</label>
				</td>
