<?php /* Template_ 2.2.6 2022/05/30 14:56:36 /www/music_brother_firstmall_kr/admin/skin/default/market_connector/market_order_list.html 000006950 */ ?>
...
<?php $this->print_("scm_login",$TPL_SCP,1);?>
		...
		<ul class="page-buttons-right">
			<li><button id="orderCollectBtn" data-mode="ORD" class="resp_btn active2 size_L">주문 수집</button></li>			
		</ul>
	</div>
</div>
<!-- //페이지 타이틀 바 -->
...
<div id="manualMatch" class="hide">
	<table class="table_basic tdc">
		<thead>
			<tr>
				<th>마켓 상품</th>
				<th><?php echo strip_tags($TPL_VAR["config_basic"]["shopName"])?> 상품코드</th>				
			</tr>		
		</thead>
		<tbody>
			<tr>
...
<div id="optionMatch" class="hide">
	<table class="table_basic tdc">
		<thead>
			<tr>
				<th>마켓 주문 옵션명</th>
				<th><?php echo strip_tags($TPL_VAR["config_basic"]["shopName"])?> 옵션명</th>				
			</tr>		
		</thead>
		<tbody>
			<tr>
	...
	<form name="openmarketAutoRegistOrderForm" id="openmarketAutoRegistOrderForm" method="post" target="actionFrame" action="../market_connector_process/openmarket_auto_regist_order">
		<table class="table_basic thl">		
			<tr>
				...
				<td>
					<div class="resp_radio">
						<label><input type="radio" name="openmarketAutoRegistOrder" value="Y" <?php if($TPL_VAR["MarketLinkage"]["openmarket_auto_regist_order"]=='Y'||$TPL_VAR["MarketLinkage"]["openmarket_auto_regist_order"]==""){?>checked<?php }?>/> 사용함</label>
						<label><input type="radio" name="openmarketAutoRegistOrder" value="N" <?php if($TPL_VAR["MarketLinkage"]["openmarket_auto_regist_order"]=='N'){?>checked<?php }?> /> 사용 안 함</label>	
					</div>					
				</td>
			</tr>
		</table>
