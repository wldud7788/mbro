<?php /* Template_ 2.2.6 2020/12/01 09:20:51 /www/music_brother_firstmall_kr/admin/skin/default/accountall/accountall_setting.html 000024981 */ ?>
<?php $this->print_("layout_header",$TPL_SCP,1);?>


<link rel="stylesheet" type="text/css" href="/admin/skin/default/css/accountall.css" />
...
<script type="text/javascript" >
	...
	$(document).ready(function() {
		...
		$("button[name='settinglist']").bind("click",function(){
			...
			openDialog("정산 마감일 변경 이력", "accountSettingListDiv", {"width":"650","height":"630"});
			if($("#accountSettingListDiv").html() == "") getSettingList(wheres);
		});
		// 정산마감일 세팅
		$("button[name='settingpop']").bind("click",function(){
			openDialog("정산 마감일 변경", "accountSettingDiv", {"width":"600","height":"280"});
		});
		// 입점사 정산주기 팝업 세팅
		$("button[name='providerselect']").bind("click",function(){
			openDialog("입점사 정산주기 변경", "accountProviderDiv", {"width":"850","height":"490"});
		});
		// 입점사 리스트 셋팅 삭제
	...
	$("input[name='orgchk[]']").each(function(){
		if($(this).is(":checked")){
			...
			setListTmp += "<tr>"
					...
					+"<td><span name=\"setprovidername\">(월"+set_provider_period+"회) "+provider_name+"("+provider_id+")</span></td>"
...
<?php if($TPL_VAR["acc_next_confirm"]){?>
	<div class="box_style_06 blue bold fx14">
		<?php echo $TPL_VAR["acc_next_confirm"]["month"]?>월부터 입점사의 정산 횟수가 (<?php if($TPL_VAR["acc_next_confirm"]["accountall_period_same"]=='N'){?>입점사별 정산 주기 상이<?php }else{?>모든 입점사 월 1회 동일<?php }?>), 정산마감일(<?php if($TPL_VAR["acc_next_confirm"]["accountall_confirm"]== 8){?>익월 7일<?php }elseif($TPL_VAR["acc_next_confirm"]["accountall_confirm"]== 11){?>익월 10일<?php }else{?>당월 말일<?php }?>)로 변경됩니다.
	</div>
<?php }?>
	
	<div class="title_dvs">
		<div class="item-title">정산 마감일 설정</div>
		<button type="button" name="settinglist" class="resp_btn v3 size_L">변경이력</button>
	</div>
	...
	<table class="table_basic thl">		
		<tr>
			<th>정산 주기</th>
			<td>
<?php if($TPL_VAR["acc_now_confirm"]["accountall_period_same"]=='Y'){?>
					모든 입점사 월 1회 동일 ( 총 입점사 <?php echo $TPL_VAR["seller_acc_cycle"][ 1]?>개 - 월 1회(<a href="#" onclick="getperiodList(1)" class="resp_btn_txt"><?php echo $TPL_VAR["seller_acc_cycle"][ 1]?></a>) )
<?php }else{?>
					입점사별 정산 주기 상이 ( 총 입점사 <?php echo $TPL_VAR["seller_acc_cycle"][ 1]+$TPL_VAR["seller_acc_cycle"][ 2]+$TPL_VAR["seller_acc_cycle"][ 4]?>개 - 월 1회(<a href="#" onclick="getperiodList(1)" class="resp_btn_txt"><?php echo $TPL_VAR["seller_acc_cycle"][ 1]?></a>), 월 2회(<a href="#" onclick="getperiodList(2)" class="resp_btn_txt"><?php echo $TPL_VAR["seller_acc_cycle"][ 2]?></a>), 월 4회(<a href="#" onclick="getperiodList(4)" class="resp_btn_txt"><?php echo $TPL_VAR["seller_acc_cycle"][ 4]?></a>) )
<?php }?>
			</td>
		</tr>
		...
		<tr>
			<th>정산 마감</th>
			<td>
<?php if($TPL_VAR["acc_now_confirm"]["accountall_confirm"]== 8){?>
					7일
<?php }elseif($TPL_VAR["acc_now_confirm"]["accountall_confirm"]== 11){?>
					10일
<?php }else{?>
					말일
<?php }?>
		...
		<tr>
			...
			<td>
				<select name="accountall_period_same" id="accountall_period_same" style="vertical-align:middle;">
					<option value="Y" <?php if($TPL_VAR["acc_next_confirm"]["accountall_period_same"]=='Y'){?>selected<?php }?>>모든 입점사 월 1회 동일</option>
					<option value="N" <?php if($TPL_VAR["acc_next_confirm"]["accountall_period_same"]=='N'){?>selected<?php }?>>입점사별 정산 주기 상이</option>
				</select>
		...
		<tr>
			...
			<td>
				<div class="resp_radio">
					<label><input type="radio" name="accountall_confirm" id="accountall_confirm_1" value="1" <?php if(($TPL_VAR["acc_next_confirm"]["accountall_confirm"]== 1)||!$TPL_VAR["acc_next_confirm"]){?>checked<?php }?>> 당월 말일</label>
					<label><input type="radio" name="accountall_confirm" id="accountall_confirm_8" value="8" <?php if($TPL_VAR["acc_next_confirm"]["accountall_confirm"]== 8){?>checked<?php }?>> 익월 7일</label>
					<label><input type="radio" name="accountall_confirm" id="accountall_confirm_11" value="11" <?php if($TPL_VAR["acc_next_confirm"]["accountall_confirm"]== 11){?>checked<?php }?>> 익월 10일</label>
				</div>
			</td>
		</tr>
		<tr>
	...
	<ul class="ul_list_01">
		<li class="wp42">
			...
			<div class="table_row_frame">					
				<div class="dvs_top">					
					<div class="dvs_right">	
						<button type="button" onclick="divExcelDownload('provider_period_list','#providerOriginalListExcel')" value="" class="resp_btn"><span class="icon_excel"></span> 다운로드</button>	
					</div>
				</div>
				<div class="scroll">
			...
			<div class="providerOriginalListExcel hide" id="providerOriginalListExcel">
				<table class="table_basic providerOriginalListTableExcel">
					<colgroup>
						<col width="20%" />
						<col width="80%" />
					</colgroup>
		...
		<li class="wp42">
			...
			<div class="table_row_frame">
				<div class="dvs_top">					
					<div class="dvs_right">	
						<button type="button" name="delProvider" class="resp_btn v3">선택 삭제</button>		
					</div>
				</div>
				<div class="scroll">
...
<div id="accountPeriodProviderDiv" class="hide">
	<div class="content">
		...
		<div class="list_info_container">
			...
			<div class="dvs_right">	
				<button type="button" onclick="divExcelDownload('provider_period_search_list','#providerPreOriginalList')" value="" class="resp_btn"><span class="icon_excel"></span> 다운로드</button>
			</div>
		</div>
		
		<div class="providerPreOriginalList" id="providerPreOriginalList">
			<table class="table_row_basic providerPreOriginalListTable">
				<colgroup>
					<col width="10%" />
					<col width="30%" />
					<col width="20%" />
