<?php /* Template_ 2.2.6 2020/12/01 09:20:51 /www/music_brother_firstmall_kr/admin/skin/default/member/email_form_dormancy.html 000011380 */ ?>
...
$(document).ready(function() {
	...
	$("#send_submit").click(function(){
		...
		if(total_count > <?php echo $TPL_VAR["mail_count"]?>){
...
function emailLogList(){
	$.get('email_log_list_pop', function(data) {
		$('#emailLogListPopup').html(data);		
		openDialog("최근 발송한 이메일", "emailLogListPopup", {"width":"800","height":"500"});
	});
}
...
function check_unsubscribe(){
	...
	$.ajax({
		type: 'get',
		url: gl_protocol+domain+'/member/unsubscribe?ussKey=checkUnsubscribe&verify=<?php echo $TPL_VAR["verify"]?>',
		dataType: 'html',
...
function copy_unsubscribe(language){
	...
	$.ajax({
		...
		data: 'language='+language+'&domain='+domain,
		dataType: 'json',
...
function excelDownloadOk(){
	...
	document.emailForm.action ="../batch_process/sms_member_download";
	document.emailForm.submit();
	document.emailForm.action ="../batch_process/send_email";
}
...
<div class="contents_container">
	<div class="content">
		...
		<table class="table_basic thl">	
			...
			<tr>
				<th>보내는 사람</th>
				<td><input type="text" name="send_email" value="<?php echo $TPL_VAR["number"]?>" style="width:600px;" title="메일 주소 입력"/></td>
			</tr>	
			<tr>
				<th>받는 사람</th>
				<td>
					<span id="search_member" class="bold">0</span>명					
					<button type="button" id="searchDormancyBtn" class="resp_btn v2">회원 검색</button></span>
