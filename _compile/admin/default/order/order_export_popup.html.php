<?php /* Template_ 2.2.6 2022/05/17 12:36:39 /www/music_brother_firstmall_kr/admin/skin/default/order/order_export_popup.html 000100458 */  $this->include_("scmSelectWarehouse");
...
<?php }?>
						...
						//개별출고 제한 @2016-03-31
						$("input.export_ea_opt").live("change",function(){
							var row = $(this).closest("tbody");
							var item_seq = $(this).attr("item_seq")
							var chk_individual_export = row.find("input[name='chk_individual_export["+item_seq+"]']").val();
						...
						// 바코드 이벤트
						$('input:[type="text"].barcode').bind('keydown', function(e){
							var mode = $(this).attr('mode')||'';
							check_barcode($(this), e, mode);
						});
					});
</script>
...
<?php }?>
	...
	table.table-export-info th.remain-ea {height:100%; background:url('/admin/skin/default/images/design/th_bg_orange.gif') repeat-x left top;}
	table.table-export-info td.remain-ea {background-color:#fefbf1 !important;}

	table.table-export-info th.export-ea {height:100%; background:url('/admin/skin/default/images/design/th_bg_blue.gif') repeat-x left top;}
...
<form name="order_export" id="goods_export" method="post" action="../order_process/order_export_popup" target="export_frame" onsubmit="loadingStart();" autocomplete="off">
	...
	<div style="padding:5px 0px 0px;" align="center">
	...
	<div class="search-form-container">
		<table class="table_search">
			<tr>
				...
				<td>
					<div class="relative">
						<input type="text" name="keyword" id="search_keyword" value="<?php echo $_GET["keyword"]?>" title="주문번호,아이디,주문자,수령자,입금자,이메일,연락처,휴대폰,상품명,상품번호,상품코드" autocomplete='off' size="100"/>
	...
	<table class="search-form-table search-detail-lay search_detail_form" id="serch_tab">
		<tr id="goods_search_form" style="display:block;"></tr>
		<tr>
			<td>
				<table class="sf-option-table table_search" border="0">

<?php if(serviceLimit('H_AD')){?>
					<tr>
						<th>배송책임</th>
						<td>
							<div class="ui-widget" id="provider_select">
								<select name="provider_seq_selector">
									<option value="">=배송책임검색=</option>
									<option value="1">본사(admin)</option>
<?php if($TPL_provider_1){foreach($TPL_VAR["provider"] as $TPL_V1){?>
									<option value="<?php echo $TPL_V1["provider_seq"]?>" <?php if($_GET["provider_seq"]==$TPL_V1["provider_seq"]){?>selected<?php }?>><?php echo $TPL_V1["provider_name"]?>(<?php echo $TPL_V1["provider_id"]?>)</option>
<?php }}?>
								</select>
								<input type="hidden" class="provider_seq" name="provider_seq" value="<?php echo $_GET["provider_seq"]?>" />
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								<input type="text" name="provider_name" value="" style="width:230px;" readonly />
							</div>
						</td>
					</tr>
<?php }?>
					<tr>
						<th>날짜</th>
						<td>
							<select name="date_field" style="width:100px;">
							...
							<input type="text" name="regist_date[]" value="<?php echo $_GET["start_search_date"]?>" class="datepicker"  maxlength="10" size="9" />
							<span class="gray">-</span>
							<input type="text" name="regist_date[]" value="<?php echo $_GET["end_search_date"]?>" class="datepicker" maxlength="10" size="9" />
							<span class="resp_btn_wrap">
							<span class="btn small"><input type="button" value="오늘" class="resp_btn select_date"  onclick="set_date('<?php echo date('Y-m-d')?>','<?php echo date('Y-m-d')?>')" style="border:0;" /></span>
							<span class="btn small"><input type="button" value="3일간" class="resp_btn select_date" onclick="set_date('<?php echo date('Y-m-d',strtotime('-3 day'))?>','<?php echo date('Y-m-d')?>')" style="border:0;" /></span>
							<span class="btn small"><input type="button" value="일주일" class="resp_btn select_date" onclick="set_date('<?php echo date('Y-m-d',strtotime('-7 day'))?>','<?php echo date('Y-m-d')?>')" style="border:0;" /></span>
							<span class="btn small"><input type="button" value="1개월" class="resp_btn select_date" onclick="set_date('<?php echo date('Y-m-d',strtotime('-1 month'))?>','<?php echo date('Y-m-d')?>')" style="border:0;" /></span>
							<span class="btn small"><input type="button" value="3개월" class="resp_btn select_date" onclick="set_date('<?php echo date('Y-m-d',strtotime('-3 month'))?>','<?php echo date('Y-m-d')?>')" style="border:0;" /></span>
							<span class="btn small"><input type="button" value="전체" class="resp_btn select_date"  onclick="set_date('','')" style="border:0;"/></span>
						</span>
						</td>
					</tr>
					<tr>
						...
						<td>
							<div class="resp_checkbox">
								<label><input type="checkbox" class="allSelectDrop" name="allshipmethod" <?php if(count($_GET['shipping_method'])== 6){?>checked<?php }?> default_none> <span class="allshipmethod fx12">전체</span></label>

<?php if($TPL_ship_set_code_1){foreach($TPL_VAR["ship_set_code"] as $TPL_K1=>$TPL_V1){?>
								<label><input type="checkbox" name="shipmethod[]" value="<?php echo $TPL_K1?>" title="<?php echo $TPL_V1?>" <?php if($_GET['shipping_method'][$TPL_K1]==$TPL_K1){?>checked<?php }?> default_none> <?php echo $TPL_V1?></label>
<?php }}?>
							</div>
						</td>
					</tr>
...
<?php if(($TPL_K1>= 40&&$TPL_K1<= 75&&substr($TPL_K1, 1, 1)== 0)||($TPL_K1>= 25&&$TPL_K1<= 35)){?>
							<label class="search_label"><input class="step" type="checkbox" name="step[<?php echo $TPL_K1?>]" value="<?php echo $TPL_K1?>" <?php if($_GET['step'][$TPL_K1]==$TPL_K1){?>checked<?php }?> /> <span class="icon-order-step-<?php echo $TPL_K1?>"><?php echo $TPL_V1?></span></label>
<?php }?>
<?php }}?>
						</span>
...
<?php if($TPL_VAR["npay_use"]||$TPL_VAR["talkbuy_use"]){?>
					<tr>
						<th>간편결제</th>
						<td>
...
<?php if($TPL_VAR["talkbuy_use"]){?>
							<label><input type="checkbox" name="search_talkbuy_order" value='y' <?php if($_GET["search_talkbuy_order"]=='y'){?>checked<?php }?>> 카카오페이 구매 주문</label>
<?php }?>
						</td>
					</tr>
<?php }?>
	...
	<div class="footer search_btn_lay">
		<div>
			<span class="sc_edit">
				...
				<button type="button" id="set_default_apply_button" onclick="set_default_search_form()" class="resp_btn v3">기본검색적용</button>
			</span>
			<span class="search">
				<button type="button" class="resp_btn active" onclick="go_search_export();"><span>검색</span></button>
...
<?php if($TPL_VAR["scm_cfg"]['use']=='Y'){?><br /><span class="desc">(공급사 배송상품은 재고 상관없이 출고 처리)</span><?php }?>
						</div>
...
<?php if($TPL_VAR["exist_provider"]&&serviceLimit('H_AD')){?>
...
<?php $this->print_("o2o_order_export_popup",$TPL_SCP,1);?>
...
<?php if($TPL_V2["request_exist"]){?>
									<input type="checkbox" name="check_shipping_seq[<?php echo $TPL_V2["shipping_seq"]?>]" class="check_shipping_group_seq" order_seq="<?php echo $TPL_V2["order_seq"]?>" value="<?php echo $TPL_V2["shipping_seq"]?>" linkage_id="<?php echo $TPL_VAR["order"][$TPL_V2["order_seq"]]["linkage_id"]?>"
<?php if($TPL_V2["request_exist"]==='pos'){?>
									disabled
<?php }else{?>
									checked="checked" onclick="check_bg();"
<?php }?>
									/>
<?php }else{?>
...
<?php }?>
								...
								<td>
									...
									<a href="javascript:printOrderView('<?php echo $TPL_V2["order_seq"]?>', 'order_export_popup')"><span class="icon-print-order"></span></a>
<?php if($TPL_V2["export_exist"]){?>
									<a href="javascript:printExportView('<?php echo $TPL_V2["order_seq"]?>', '', 'order_export_popup')"><span class="icon-print-export"></span></a>
<?php }?>
...
<?php if($TPL_V2["talkbuy_order_id"]){?><div class="kyellow bold"><?php echo $TPL_V2["talkbuy_order_id"]?><span style="font-size:11px;font-weight:normal"> (Kpay주문번호)</span></div><?php }?>
...
<?php if($TPL_VAR["order"][$TPL_V2["order_seq"]]["linkage_id"]=='pos'){?>
									<input type="hidden" name="direct_export_complete[<?php echo $TPL_V2["shipping_seq"]?>]" value="1"/>
									<div class="ngreen bold">
										<?php echo $TPL_VAR["order"][$TPL_V2["order_seq"]]["linkage_order_id"]?>

										<span style="font-size:11px;font-weight:normal">
									(<?php echo $TPL_VAR["order"][$TPL_V2["order_seq"]]["linkage_mallname_text"]?>)
								</span>
									</div>
<?php }?>
								</td>
...
<?php if($TPL_V2["shipping_method"]!='coupon'&&$TPL_V2["request_exist"]){?>
						<input type="text" name="barcode[<?php echo $TPL_V2["shipping_seq"]?>]" style="width:100%;" value="" title="바코드 스캔" class="barcode" tabindex="<?php echo $TPL_V2["num"]?>" <?php if($TPL_VAR["data_search_default"]["order_detail_view"]=='close'){?>disabled="disabled"<?php }?> />
<?php }?>
					</td>
					<td width="7%" align="right">
<?php if($TPL_V2["request_exist"]){?>
						<span class="btn small cyanblue" ><button type="button" id="goods_export"
<?php if($TPL_V2["request_exist"]==='pos'){?>
						onclick="alert('미매칭 상품이 포함된 주문은 출고가 불가합니다. 상품 매칭 후 다시 시도해주세요.');"
<?php }else{?>
						onclick="export_each_popup('<?php echo $TPL_V2["shipping_seq"]?>','<?php echo current(array_keys($TPL_V2["options"]))?>','<?php echo $TPL_V2["shipping_method"]?>');"
<?php }?>
					>출고처리></button></span>
<?php }else{?>
...
<?php if($TPL_I1== 0&&$TPL_I2== 0){?>
			<tr>
				<th>주문상품</th>
				<th><?php echo scmSelectWarehouse($TPL_VAR["shopSno"],$TPL_VAR["scmOption"])?>재고</th>
				<th>주문</th>
				<th>취소</th>
				<th>보낸수량</th>
				<th>남은수량 </th>
					...
					<table class="list-goods-info">
						<tr>
							<td width="50" style="border:0px;">
								<span class="order-item-image"><img src="<?php echo $TPL_V2["items"][$TPL_V3["item_seq"]]["image"]?>" class="small_goods_image" onerror="this.src='/admin/skin/default/images/common/noimage_list.gif';" /></span>
							</td>
							<td style="border:0px;">
...
<?php if($TPL_V3["talkbuy_product_order_id"]){?><div class="kyellow bold"><?php echo $TPL_V3["talkbuy_product_order_id"]?><span style="font-size:11px;font-weight:normal"> (Kpay상품주문번호)</span></div><?php }?>
...
<?php }?>
								<div class="blue">
									...
									<span class="hand shipping_set_name_<?php echo $TPL_V2["shipping_seq"]?>"
<?php if(($TPL_VAR["order"][$TPL_V2["order_seq"]]["linkage_id"]=='pos')){?>
									onclick="alert('오프라인 주문은 매장을 변경할 수 없습니다.<?php echo $TPL_VAR["order"][$TPL_V2["order_seq"]]["linkage_id"]?>');"
<?php }elseif(($TPL_VAR["order"][$TPL_V2["order_seq"]]["pg"]=='talkbuy')){?>
									onclick='openDialog("카카오페이 구매", "talkbuy_delivary_dialog", {"width":"450","height":"150"});'
<?php }else{?>
									onclick="ship_chg_popup('<?php echo $TPL_V2["shipping_seq"]?>','order','after');"
<?php }?>
									>
									<?php echo $TPL_V2["shipping_set_name"]?>
...
<?php if($TPL_VAR["talkbuy_use"]&&$TPL_V3["talkbuy_product_order_id"]&&$TPL_V3["top_item_option_seq"]&&$TPL_V2["talkbuy_flag_msg"]){?>
								<label>
									<input type="checkbox" name="kpay_flag_release[<?php echo $TPL_V2["shipping_seq"]?>]" value='<?php echo $TPL_V3["exchange_return_code"]?>'>
									<span class='desc'><span class="red">보류해제(사유:<?php echo $TPL_V3["talkbuy_flag_msg"]?>)</span></span>
								</label>
<?php }?>
...
<?php if($TPL_VAR["order"][$TPL_V2["order_seq"]]["linkage_id"]=='pos'){?>
<?php if($TPL_V4["shipping_address_seq"]==$TPL_V2["shipping_address_seq"]){?>
...
<?php if($TPL_VAR["scm_cfg"]['use']=='Y'&&$TPL_V2["items"][$TPL_V3["item_seq"]]["goods_data"]["provider_seq"]== 1&&$TPL_V3["package_yn"]!='y'){?>
					<input type="hidden" name="optioninfo[<?php echo $TPL_V2["shipping_seq"]?>][option][<?php echo $TPL_V3["item_option_seq"]?>]" class="optioninfo" value="<?php echo $TPL_V2["items"][$TPL_V3["item_seq"]]["goods_data"]["goods_seq"]?>option<?php echo $TPL_V3["option_seq"]?>" />
					<input type="hidden" name="whSupplyPrice[<?php echo $TPL_V2["shipping_seq"]?>][option][<?php echo $TPL_V3["item_option_seq"]?>]" class="wh_supply_price" value="<?php echo $TPL_V3["supply_price"]?>" />
					<input type="hidden" name="goodscode[<?php echo $TPL_V2["shipping_seq"]?>][option][<?php echo $TPL_V3["item_option_seq"]?>]" value="<?php echo $TPL_V3["goods_code"]?>" />
<?php }?>
...
<?php if($TPL_V3["package_yn"]!='y'){?>
								<div class="warehouse-info-lay warehouse_<?php echo $TPL_V2["shipping_seq"]?>_option_<?php echo $TPL_V3["item_option_seq"]?>">
									<ul>
										<li class="wh_info"></li>
...
<?php }?>
<?php if($TPL_V3["talkbuy_pay_delivery"]=='y'){?>
								<div class='red'>! 카카오페이 구매 판매자센터에서 출고진행중인 주문입니다.</div>
<?php }?>
							</td>
						</tr>
					</table>
...
<?php }?>
					...
					<input type="hidden" name="autoWh[<?php echo $TPL_V2["shipping_seq"]?>][option][<?php echo $TPL_V3["item_option_seq"]?>]" class="auto_warehousing" value="0" />
<?php }else{?>
...
<?php }else{?>
					...
					<input type="hidden" name="package_grouping_key[]" value="package-<?php echo $TPL_V3["item_option_seq"]?>">
					<!--패키지묶음을 구분하기위한 key로 -->
<?php }?>
				</td>
...
<?php if($TPL_V3["request_ea"]== 0){?>
					0
<?php if($TPL_V2["items"][$TPL_V3["item_seq"]]["goods_data"]["goods_kind"]=='coupon'){?>
					<input type="hidden" name="request_ea[<?php echo $TPL_V2["shipping_seq"]?>][option][<?php echo $TPL_V3["item_option_seq"]?>]" class="line"  style="text-align:right;background-color:#efefef;" size="2" value="<?php echo $TPL_V3["request_ea"]?>" org="<?php echo $TPL_V3["request_ea"]?>" opt_type="opt" item_seq="<?php echo $TPL_V3["item_seq"]?>" readonly>
					<input type="hidden" name="shipping_goods_kind[<?php echo $TPL_V2["shipping_seq"]?>][option][<?php echo $TPL_V3["item_option_seq"]?>]"  value="COU" />
<?php }else{?>
					<input type="hidden" name="request_ea[<?php echo $TPL_V2["shipping_seq"]?>][option][<?php echo $TPL_V3["item_option_seq"]?>]" class="line"  style="text-align:right" size="2" value="<?php echo $TPL_V3["request_ea"]?>" onkeydown="reset_barcode_ea(this);" org="<?php echo $TPL_V3["request_ea"]?>" opt_type="opt" item_seq="<?php echo $TPL_V3["item_seq"]?>" >
					<input type="hidden" name="shipping_goods_kind[<?php echo $TPL_V2["shipping_seq"]?>][option][<?php echo $TPL_V3["item_option_seq"]?>]"  value="OPT" />
<?php }?>
<?php }else{?>
<?php if($TPL_V2["items"][$TPL_V3["item_seq"]]["goods_data"]["goods_kind"]=='coupon'){?>
					...
					<input type="hidden" name="shipping_goods_kind[<?php echo $TPL_V2["shipping_seq"]?>][option][<?php echo $TPL_V3["item_option_seq"]?>]"  value="COU" />
<?php }else{?>
<?php if(($TPL_VAR["npay_use"]&&$TPL_V3["npay_product_order_id"])||($TPL_VAR["talkbuy_use"]&&$TPL_V3["talkbuy_product_order_id"])||$TPL_VAR["order"][$TPL_V2["order_seq"]]["linkage_id"]=='connector'||$TPL_VAR["order"][$TPL_V2["order_seq"]]["linkage_id"]=='pos'){?>
					<input type="hidden" name="request_ea[<?php echo $TPL_V2["shipping_seq"]?>][option][<?php echo $TPL_V3["item_option_seq"]?>]" class="line export_ea export_ea_opt" value="<?php echo $TPL_V3["request_ea"]?>"  org="<?php echo $TPL_V3["request_ea"]?>" opt_type="opt" item_seq="<?php echo $TPL_V3["item_seq"]?>">
					<input type="text" name="request_ea_tmp[<?php echo $TPL_V2["shipping_seq"]?>][option][<?php echo $TPL_V3["item_option_seq"]?>]" class="line export_ea_tmp"  style="text-align:right" size="2" value="<?php echo $TPL_V3["request_ea"]?>" disabled style="background-color:#eee;">
<?php }else{?>
					<input type="text" name="request_ea[<?php echo $TPL_V2["shipping_seq"]?>][option][<?php echo $TPL_V3["item_option_seq"]?>]" class="line export_ea export_ea_opt" style="text-align:right" size="2" value="<?php echo $TPL_V3["request_ea"]?>" onkeyup="reset_barcode_ea(this);check_request_ea(this,<?php echo $TPL_V3["request_ea"]?>);" org="<?php echo $TPL_V3["request_ea"]?>" opt_type="opt" item_seq="<?php echo $TPL_V3["item_seq"]?>" >
<?php }?>
					<input type="hidden" name="shipping_goods_kind[<?php echo $TPL_V2["shipping_seq"]?>][option][<?php echo $TPL_V3["item_option_seq"]?>]"  value="OPT" />
<?php }?>
<?php }?>
					<!--패키지묶음을 구분하기위한 key 정의 -->
					<input type="hidden" name="package_grouping_key[]" value="package-<?php echo $TPL_V3["item_option_seq"]?>">
				</td>
<?php }?>
				<td class="info option" align="center">
					<span class="fx11 dotum"><?php if($TPL_V3["request_ea"]== 0||$TPL_V3["package_yn"]=='y'){?>-<?php }else{?>대기<?php }?></span>
<?php if($TPL_V3["request_ea"]> 0&&$TPL_V3["package_yn"]!='y'){?>
...
<?php if($TPL_I3== 0){?>
				<td class="info option delivery-info" rowspan="<?php echo $TPL_V2["rowspan"]?>">
<?php if($TPL_VAR["data_search_default"]["order_detail_view"]!='close'){?>
					<table border="0" cellpadding="0" cellspacing="0" width="100%" class="inner-table shipping_info_<?php echo $TPL_V2["shipping_seq"]?>">
						<tr>
<?php if(preg_match('/coupon/',$TPL_V2["shipping_method"])){?>
...
<?php if(($TPL_VAR["order"][$TPL_V2["order_seq"]]["linkage_id"]!='pos')){?>
									onclick="ship_chg_popup('<?php echo $TPL_V2["shipping_seq"]?>','order','after');"
<?php }else{?>
									onclick="alert('오프라인 주문은 매장을 변경할 수 없습니다.');"
<?php }?>
									>
									<?php echo $TPL_V2["shipping_set_name"]?>
...
<?php if(is_array($TPL_R4=$TPL_V3["packages"])&&!empty($TPL_R4)){$TPL_I4=-1;foreach($TPL_R4 as $TPL_V4){$TPL_I4++;?>
			<tr class="open-tr package-<?php echo $TPL_V3["item_option_seq"]?>">
...
<?php if($TPL_VAR["scm_cfg"]['use']=='Y'&&$TPL_V2["items"][$TPL_V3["item_seq"]]["goods_data"]["provider_seq"]== 1){?>
					<input type="hidden" name="packageOptioninfo[<?php echo $TPL_V2["shipping_seq"]?>][option][<?php echo $TPL_V3["item_option_seq"]?>][<?php echo $TPL_V4["package_option_seq"]?>]" class="optioninfo" value="<?php echo $TPL_V4["goods_seq"]?>option<?php echo $TPL_V4["option_seq"]?>" />
					<input type="hidden" name="packageGoodscode[<?php echo $TPL_V2["shipping_seq"]?>][option][<?php echo $TPL_V3["item_option_seq"]?>][<?php echo $TPL_V4["package_option_seq"]?>]" value="<?php echo $TPL_V4["goods_code"]?>" />
					<input type="hidden" name="packageWhSupplyPrice[<?php echo $TPL_V2["shipping_seq"]?>][option][<?php echo $TPL_V3["item_option_seq"]?>][<?php echo $TPL_V4["package_option_seq"]?>]" class="wh_supply_price" value="<?php echo $TPL_V4["supply_price"]?>" />
<?php }?>
					<table class="list-goods-info" width="100%">
						<tr>
...
<?php }?>
								...
								<div class="warehouse-info-lay warehouse_<?php echo $TPL_V2["shipping_seq"]?>_option_<?php echo $TPL_V3["item_option_seq"]?>_<?php echo $TPL_V4["package_option_seq"]?>">
									<ul>
										<li class="wh_info"></li>
...
<?php }else{?>
						<span class="red stock"><?php echo number_format($TPL_V4["stock"])?></span>
						<input type="hidden" name="packageStock[<?php echo $TPL_V2["shipping_seq"]?>][option][<?php echo $TPL_V3["item_option_seq"]?>][<?php echo $TPL_V4["package_option_seq"]?>]" class="whstock" value="<?php echo $TPL_V4["stock"]?>" unit="<?php echo $TPL_V4["unit_ea"]?>" />
						<input type="hidden" name="packageAutoWh[<?php echo $TPL_V2["shipping_seq"]?>][option][<?php echo $TPL_V3["item_option_seq"]?>][<?php echo $TPL_V4["package_option_seq"]?>]" class="auto_warehousing" value="0" />
<?php }?>
				</td>
...
<?php }?>
				...
				<td class="info option center">
					...
					<input type="hidden" class="orderEA" value="<?php echo $TPL_V3["request_ea"]*$TPL_V4["unit_ea"]?>" />
...
<?php if($TPL_V3["request_ea"]> 0){?>
					<input type="text" name="package_barcode_ea[<?php echo $TPL_V2["shipping_seq"]?>][<?php echo strtoupper($TPL_V4["bar_goods_code"])?>]" class="barcode_ea_<?php echo $TPL_V2["shipping_seq"]?> barcode_ready package" style="text-align:right;" value="0" size="2" onblur="package_check_barcode_ea(this);" />
<?php }?>
				</td>
			</tr>
<?php }}?>
...
<?php if(is_array($TPL_R4=$TPL_V3["suboptions"])&&!empty($TPL_R4)){foreach($TPL_R4 as $TPL_V4){?>
			<tr class="open-tr">
				<td class="info suboption" style="padding:3px 0px 0px 55px;">
<?php if($TPL_VAR["scm_cfg"]['use']=='Y'&&$TPL_V2["items"][$TPL_V3["item_seq"]]["goods_data"]["provider_seq"]== 1&&$TPL_V4["package_yn"]!='y'){?>
					<input type="hidden" name="optioninfo[<?php echo $TPL_V2["shipping_seq"]?>][suboption][<?php echo $TPL_V4["item_suboption_seq"]?>]"  class="optioninfo" value="<?php echo $TPL_V2["items"][$TPL_V3["item_seq"]]["goods_data"]["goods_seq"]?>suboption<?php echo $TPL_V4["suboption_seq"]?>" />
					<input type="hidden" name="goodscode[<?php echo $TPL_V2["shipping_seq"]?>][suboption][<?php echo $TPL_V4["item_suboption_seq"]?>]" value="<?php echo $TPL_V4["goods_code"]?>" />
					<input type="hidden" name="whSupplyPrice[<?php echo $TPL_V2["shipping_seq"]?>][suboption][<?php echo $TPL_V4["item_suboption_seq"]?>]" class="wh_supply_price" value="<?php echo $TPL_V4["supply_price"]?>" />
<?php }?>
...
<?php if($TPL_V4["package_yn"]!='y'){?>
					<div class="warehouse-info-lay warehouse_<?php echo $TPL_V2["shipping_seq"]?>_suboption_<?php echo $TPL_V4["item_suboption_seq"]?>">
						<ul>
							<li class="wh_info"></li>
							<li>상품코드 : <?php echo $TPL_V4["goods_code"]?></li>
						</ul>
...
<?php }?>
					...
					<input type="hidden" name="autoWh[<?php echo $TPL_V2["shipping_seq"]?>][suboption][<?php echo $TPL_V4["item_suboption_seq"]?>]" class="auto_warehousing" value="0" />
<?php }else{?>
					<span class="fx11 dotum">실제상품▼</span>
<?php }?>
				</td>
...
<?php if($TPL_V2["items"][$TPL_V3["item_seq"]]["goods_data"]["goods_kind"]!='coupon'){?>
<?php if(($TPL_VAR["npay_use"]&&$TPL_V3["npay_product_order_id"])||($TPL_VAR["talkbuy_use"]&&$TPL_V3["talkbuy_product_order_id"])||$TPL_VAR["order"][$TPL_V2["order_seq"]]["linkage_id"]=='connector'||$TPL_VAR["order"][$TPL_V2["order_seq"]]["linkage_id"]=='pos'){?>
					<input type="hidden" name="request_ea[<?php echo $TPL_V2["shipping_seq"]?>][suboption][<?php echo $TPL_V4["item_suboption_seq"]?>]" class="line export_ea" value="<?php echo $TPL_V4["request_ea"]?>" org="<?php echo $TPL_V4["request_ea"]?>" opt_type="sub" item_seq="<?php echo $TPL_V3["item_seq"]?>">
					<input type="text" name="request_ea_tmp[<?php echo $TPL_V2["shipping_seq"]?>][suboption][<?php echo $TPL_V4["item_suboption_seq"]?>]" class="line export_ea_tmp"  style="text-align:right" size="2" value="<?php echo $TPL_V4["request_ea"]?>" org="<?php echo $TPL_V4["request_ea"]?>" disabled>
<?php }else{?>
...
<?php }?>
					<!--패키지묶음을 구분하기위한 key 정의 -->
					<input type="hidden" name="package_grouping_key[]" value="package-<?php echo $TPL_V4["item_suboption_seq"]?>">
				</td>
<?php }?>
				<td class="info suboption" align="center">
					<span class="fx11 dotum"><?php if($TPL_V4["request_ea"]== 0||$TPL_V4["package_yn"]=='y'){?>-<?php }else{?>대기<?php }?></span>
<?php if($TPL_V4["request_ea"]> 0&&$TPL_V4["package_yn"]!='y'){?>
...
<?php if(is_array($TPL_R5=$TPL_V4["packages"])&&!empty($TPL_R5)){foreach($TPL_R5 as $TPL_V5){?>
			<tr class="open-tr package-<?php echo $TPL_V4["item_suboption_seq"]?>">
				<td class="info suboption" style="padding-left:68px;">
<?php if($TPL_VAR["scm_cfg"]['use']=='Y'&&$TPL_V2["items"][$TPL_V3["item_seq"]]["goods_data"]["provider_seq"]== 1){?>
					<input type="hidden" name="packageOptioninfo[<?php echo $TPL_V2["shipping_seq"]?>][suboption][<?php echo $TPL_V4["item_suboption_seq"]?>][<?php echo $TPL_V5["package_suboption_seq"]?>]" class="optioninfo" value="<?php echo $TPL_V5["goods_seq"]?>option<?php echo $TPL_V5["option_seq"]?>" />
					<input type="hidden" name="packageGoodscode[<?php echo $TPL_V2["shipping_seq"]?>][suboption][<?php echo $TPL_V4["item_suboption_seq"]?>][<?php echo $TPL_V5["package_suboption_seq"]?>]" value="<?php echo $TPL_V5["goods_code"]?>" />
					<input type="hidden" name="packageWhSupplyPrice[<?php echo $TPL_V2["shipping_seq"]?>][suboption][<?php echo $TPL_V4["item_suboption_seq"]?>][<?php echo $TPL_V5["package_suboption_seq"]?>]" class="wh_supply_price" value="<?php echo $TPL_V5["supply_price"]?>" />
<?php }?>
					<table class="list-goods-info" width="100%">
						<tr>
							<td valign="top" style="border:none;"><img src="/admin/skin/default/images/common/icon/ico_package.gif" border="0" /></td>
...
<?php }?>
								...
								<div class="warehouse-info-lay warehouse_<?php echo $TPL_V2["shipping_seq"]?>_suboption_<?php echo $TPL_V4["item_suboption_seq"]?>_<?php echo $TPL_V5["package_suboption_seq"]?>">
									<ul>
										<li class="wh_info"></li>
...
<?php if($TPL_V5["stock"]==='미매칭'){?>
					<div class="red">미매칭</span>
<?php }else{?>
						<span class="red stock"><?php echo number_format($TPL_V5["stock"])?></span>
						<input type="hidden" name="packageStock[<?php echo $TPL_V2["shipping_seq"]?>][suboption][<?php echo $TPL_V4["item_suboption_seq"]?>][<?php echo $TPL_V5["package_suboption_seq"]?>]" class="whstock" value="<?php echo $TPL_V5["stock"]?>" unit="<?php echo $TPL_V5["unit_ea"]?>" />
						<input type="hidden" name="packageAutoWh[<?php echo $TPL_V2["shipping_seq"]?>][suboption][<?php echo $TPL_V4["item_suboption_seq"]?>][<?php echo $TPL_V5["package_suboption_seq"]?>]" class="auto_warehousing" value="0" />
<?php }?>
				</td>
...
<?php }?>
				...
				<td class="info suboption center">
					...
					<input type="hidden" class="orderEA" value="<?php echo $TPL_V5["unit_ea"]*$TPL_V4["request_ea"]?>" />
					<span class="helpicon" title="해당 판매상품 주문수량 1개일때 해당 실제상품의 발송수량"></span>
				</td>
				<td class="info suboption center">
				<span class="fx11 dotum">
			...
			<table width="100%" border="0" style="table-layout: fixed">
				<tr>
					...
					<td width="29%" class="left">
						<input type="text" name="bundle_barcode" style="width:100%;" value="" title="바코드 스캔" mode="bundle" class="barcode"/>
					</td>
...
<?php }?>
		...
		<div class="center blue pdb5 fx11">
			합포장(묶음배송)으로 인해 배송비를 고객님께 되돌려 드려야 할 경우 환불리스트에서 환불신청(배송비)을 등록하실 수 있습니다.
		</div>
...
<?php }?>
	...
	<table  width="100%">
		<tr>
			...
			<td>
				<span class="btn large cyanblue" ><button type="button" id="bundle_goods_export" style="width:120px;" onclick="export_submit('bundle');">합포장 출고처리</button></span>
			</td>
		</tr>
	</table>
	<br /><br />
