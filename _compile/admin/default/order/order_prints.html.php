<?php /* Template_ 2.2.6 2022/05/17 12:36:40 /www/music_brother_firstmall_kr/admin/skin/default/order/order_prints.html 000037706 */ 
$TPL_loop_1=empty($TPL_VAR["loop"])||!is_array($TPL_VAR["loop"])?0:count($TPL_VAR["loop"]);?>
<?php $this->print_("layout_header_popup",$TPL_SCP,1);?>

<link rel="stylesheet" type="text/css" href="/admin/skin/default/css/orderprint.css" />
		...
		<td>
			<table width="100%" class="order_print_info" cellspacing="0" cellpadding="0" border="0">
				<tr>
					<th>주문번호</th>
					<td>
						<?php echo $TPL_V1["order"]["order_seq"]?>

<?php if($TPL_V1["order"]["linkage_mall_order_id"]){?>
						/ <?php echo $TPL_V1["order"]["linkage_mall_order_id"]?> (<?php echo $TPL_V1["order"]["linkage_mallname_text"]?>)
<?php }elseif($TPL_V1["order"]["npay_order_id"]){?>
						/ <?php echo $TPL_V1["order"]["npay_order_id"]?> (Npay주문번호)
<?php }elseif($TPL_V1["order"]["talkbuy_order_id"]){?>
						/ <?php echo $TPL_V1["order"]["talkbuy_order_id"]?> (Kpay주문번호)
<?php }elseif($TPL_V1["order"]["linkage_id"]=='pos'){?>
						/ <?php echo $TPL_V1["order"]["linkage_order_id"]?> (<?php echo $TPL_V1["order"]["linkage_mallname_text"]?>)
<?php }?>
					</td>
				</tr>
				<tr>
...
<?php if($TPL_V1["order"]["deposit_yn"]!='y'){?><span>(입금전)</span><?php }?>
				...
				<tr>
					<th>할인</th>
					<td><?php echo get_currency_price($TPL_V1["items_tot"]["event_sale"]+$TPL_V1["items_tot"]["multi_sale"]+$TPL_V1["items_tot"]["coupon_sale"]+$TPL_V1["items_tot"]["member_sale"]+$TPL_V1["items_tot"]["fblike_sale"]+$TPL_V1["items_tot"]["mobile_sale"]+$TPL_V1["items_tot"]["promotion_code_sale"]+$TPL_V1["items_tot"]["referer_sale"]+$TPL_V1["shipping_tot"]["shipping_coupon_sale"]+$TPL_V1["shipping_tot"]["shipping_promotion_code_sale"]+$TPL_V1["order"]["enuri"], 4)?></td>
				</tr>
				<tr>
					<th>사용 캐시/예치금</th>
...
<?php }?>
	...
	<tr>
		<th>배송 메시지</th>
		<td colspan="5"><?php if($TPL_V1["order"]["each_msg_yn"]=='Y'){?><?php if(is_array($TPL_R2=$TPL_V1["order"]["memo"]["ship_message"])&&!empty($TPL_R2)){$TPL_I2=-1;foreach($TPL_R2 as $TPL_K2=>$TPL_V2){$TPL_I2++;?><?php echo $TPL_K2+ 1?>. <?php echo $TPL_V1["order"]["memo"]["goods_info"][$TPL_I2]?> : [<?php echo $TPL_V2?>]<BR><?php }}?><?php }else{?><?php echo $TPL_V1["order"]["memo"]?><?php }?></td>
	</tr>
	</table>
	...
	<table align="center" width="98%" cellspacing="0" cellpadding="0" border="0">
	<tr>
		<td class="left"><span class="title_print_info">주문상품</span>(<?php echo number_format($TPL_V1["order"]["total_type"])?>종-총 <?php echo number_format($TPL_V1["order"]["total_ea"])?>개)</td>
	</tr>
	</table>
...
<?php if($TPL_V7["type"]=='file'){?>
									<a href="/mypage_process/filedown?file=<?php echo $TPL_V5["value"]?>" target="actionFrame"><?php echo $TPL_V7["value"]?></a>
