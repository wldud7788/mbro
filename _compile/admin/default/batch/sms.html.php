<?php /* Template_ 2.2.6 2022/01/05 11:39:08 /www/music_brother_firstmall_kr/admin/skin/default/batch/sms.html 000017471 */ 
...
<style>
	.title_dvs > .resp_btn, .title_dvs > .btn_resp, .title_dvs > .resp_btn_dvs{right: calc(5% - 10px);}
</style>
...
<input type="hidden" name="sms_id" value="<?php echo $TPL_VAR["chk"]?>">
<input type="hidden" name="requestToken" value="<?php echo $TPL_VAR["requestToken"]?>">
<input type="hidden" name="accessToken" value="<?php echo $TPL_VAR["accessToken"]?>">
...
<?php if($TPL_VAR["send_phone"]){?><?php echo $TPL_VAR["send_phone"]?><?php }else{?>등록된 발신번호가 없습니다.<?php }?>
		...
		<tr>
			...
			<td>
				...
				<table class="displayTabList table_basic tdc wx400 mt5" >
					<colgroup>
						<col width="80%" />					
						<col width="20%" />						
					</colgroup>
...
<?php }}?>
	...
	<div class="footer"><button type="button" id="send_submit" <?php if($TPL_VAR["count"]< 1){?>disabled<?php }?> class="resp_btn active size_XL">발송</button></div>
</div>
</form>
<!-- 여기서 부터 기존 폼-->
...
<?php if($TPL_I1% 5== 0){?></tr><tr><?php }?>
						<td class="its-td-align" style="border-top:1px solid #dddddd;border-right:1px solid #dddddd;text-align:center;width:20%;"><?php if($TPL_I1== 0){?><a href="javascript:loadSmsForm('');"><?php }else{?><a href="javascript:loadSmsForm('<?php echo $TPL_V1["category"]?>');"><?php }?><?php echo $TPL_V1["category"]?></a></td>
<?php }}?>
					</tr>
					</table>
