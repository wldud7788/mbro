<?php /* Template_ 2.2.6 2020/12/01 09:20:51 /www/music_brother_firstmall_kr/admin/skin/default/setting/lg.html 000024346 */ 
...
$TPL_arrLgCardCompany_1=empty($TPL_VAR["arrLgCardCompany"])||!is_array($TPL_VAR["arrLgCardCompany"])?0:count($TPL_VAR["arrLgCardCompany"]);?>
...
<div class="clearbox inputPgSetting">
	...
	<table width="100%" class="table_basic" height="1000px">
...
<?php }else{?>
					<input type="radio" class="line" name="nonActiveXUse" id="nonActiveXNot" size="40" value="N" title="웹표준 결제 사용안함" <?php if($TPL_VAR["nonActiveXUse"]==""||$TPL_VAR["nonActiveXUse"]=="N"){?> checked = "cehcked" <?php }?>/> <label for="nonActiveXNot">XPay</label><br/>			
					<input type="radio" class="line" name="nonActiveXUse" id="nonActiveXUse" size="40" value="Y" title="웹표준 결제 사용" <?php if($TPL_VAR["nonActiveXUse"]=="Y"){?> checked = "cehcked" <?php }?>/> <label for="nonActiveXUse">[NON-ActiveX] XPayCrossplatform</label>
<?php }?>			
			</td>
		</tr>
		<tr>
			...
			<td class="its-td">
				<div><input type="text" name="mallCode" class="line" value="<?php echo $TPL_VAR["mallCode"]?>" title="쇼핑몰 코드" /></div>
				<div class="desc">
				쇼핑몰 코드는 <span class="bold black">gb_</span>로 시작하는 표준코드만 입력이 가능합니다.
				</div>
				<div style="padding-top:5px;"><input type="text" class="line" name="merchantKey" size="40" value="<?php echo $TPL_VAR["merchantKey"]?>" title="쇼핑몰 머천트키" /></div>
			</td>
		</tr>
		<tr>
...
<?php if($TPL_arrLgCardCompany_1){foreach($TPL_VAR["arrLgCardCompany"] as $TPL_V1){?>
