<?php /* Template_ 2.2.6 2022/05/17 12:37:01 /www/music_brother_firstmall_kr/admin/skin/default/setting/sale.html 000023442 */ ?>
...
<?php if($TPL_VAR["cashreceiptauto"]){?>
			$("input[name='cashreceiptauto'][value='<?php echo $TPL_VAR["cashreceiptauto"]?>']").attr('checked',true);
<?php }?>

<?php if($TPL_VAR["biztype"]){?>
			$("input[name='biztype'][value='<?php echo $TPL_VAR["biztype"]?>']").attr('checked',true);
			
<?php if($TPL_VAR["biztype"]=="tax"){?>			
				$(".webmailDetail").show();
<?php }else{?>
				$(".webmailDetail").hide();
<?php }?>
...
<?php $this->print_("require_info",$TPL_SCP,1);?>
			...
			<div class="page-buttons-right">
				<button class="resp_btn active2 size_L" <?php if($TPL_VAR["isdemo"]["isdemo"]){?>  type="button" <?php echo $TPL_VAR["isdemo"]["isdemojs1"]?> <?php }else{?> type="submit" <?php }?>>저장</button>
			</div>
		</div>
	</div>
	<!-- 페이지 타이틀 바 : 끝 -->
				...
				<tr class="webmailDetail">
					...
					<td class="clear">
						<table width="100%" class="table_basic v3 thl">								
							<tr>
								<th>하이웍스 개설 오피스주소</th>
								<td><input type="text" name="webmail_domain" value="<?php echo $TPL_VAR["webmail_domain"]?>" size="50" <?php if($TPL_VAR["isdemo"]["isdemo"]){?> <?php echo $TPL_VAR["isdemo"]["isdemodisabled"]?> <?php }?> ></td>
							</tr>
							<tr>
								<th>사용자 ID</th>
								<td><input type="text" name="webmail_admin_id" value="<?php echo $TPL_VAR["webmail_admin_id"]?>" size="50" <?php if($TPL_VAR["isdemo"]["isdemo"]){?> <?php echo $TPL_VAR["isdemo"]["isdemodisabled"]?> <?php }?> ></td>
							</tr>
							<tr>
								<th>연동 KEY</th>
								<td><input type="text" name="webmail_key" value="<?php echo $TPL_VAR["webmail_key"]?>" size="50" <?php if($TPL_VAR["isdemo"]["isdemo"]){?> <?php echo $TPL_VAR["isdemo"]["isdemodisabled"]?> <?php }?> /></td>
							</tr>
						</table>
...
<?php }?>
				...
				<tr class="cashreceiptauto">
					...
					<td>
						<div class="resp_radio">
							...
							( <input type="text" name="cashreceiptautoprice" value="<?php echo $TPL_VAR["cashreceiptautoprice"]?>" class="onlynumber" /> 원 이상 시 의무 발행)
			...
			<ul class="bullet_hyphen resp_message">
				<li>현금 영수증 중복 발행을 유의해주세요.<span class="tooltip_btn" onClick="showTooltip(this, '/admin/tooltip/sale', '#tip3', 'sizeR')"></li>	
		...
		<div class="box_style_05 mt30">
			...
			<ul class="bullet_circle">
				...
				<li>모든 매출증빙 자료는 주문 > <a href="/admin/order/sales" class="link_blue_01" >매출증빙리스트</a>에서 확인할 수 있습니다.</li>
...
<div id="pg_sales_guide" class="hide">
	<div style="width:903px;">
		<table width="100%" cellpadding="0" cellspacing="0">
			...
			<tr>
				<td><img src="/admin/skin/default/images/common/img_setting_taxation01.gif"></td>
			</tr>
			<tr><td height="20"></td></tr>
			<tr>
				<td><img src="/admin/skin/default/images/common/img_setting_taxation02.gif"></td>
			</tr>
			<tr><td height="20"></td></tr>
			<tr>
				<td><img src="/admin/skin/default/images/common/img_setting_taxation03.gif"></td>
			</tr>
		</table>
