<?php /* Template_ 2.2.6 2020/12/01 09:20:51 /www/music_brother_firstmall_kr/admin/skin/default/setting/kspay.html 000023608 */ 
...
$TPL_arrKspayCompany_1=empty($TPL_VAR["arrKspayCompany"])||!is_array($TPL_VAR["arrKspayCompany"])?0:count($TPL_VAR["arrKspayCompany"]);?>
...
<div class="clearbox inputPgSetting">
	...
	<table width="100%" class="table_basic" style="table-layout:fixed;">
		...
		<tr>
			<th class="its-th" rowspan="6" style="min-width:100px;">일반</th>
		...
		<tr>
			<td class="its-td" style="min-width:120px;">세팅 정보 등록</td>
			<td class="its-td">
				<div><input type="text" name="mallId" class="line" value="<?php echo $TPL_VAR["mallId"]?>" title="상점ID" /></div>
				<div class="desc">상점 아이디는 <span class="bold black">23, 25, 26, 28</span>로 시작하는 표준코드만 입력이 가능합니다.</div>
				<div style="padding-top:5px;"><input type="password" class="line" name="mallPass" size="40" value="<?php echo $TPL_VAR["mallPass"]?>" title="상점비밀번호" /></div>
			</td>
		</tr>
		<tr>
...
<?php if($TPL_pcCardCompanyCode_1){$TPL_I1=-1;foreach($TPL_VAR["pcCardCompanyCode"] as $TPL_V1){$TPL_I1++;?>
					<div>
					- <?php echo $TPL_VAR["arrKspayCompany"][$TPL_V1]?> <?php echo $TPL_VAR["pcCardCompanyTerms"][$TPL_I1]?> 개월
...
<?php }}?>
		...
		<tr>
			...
			<td class="its-td">
				...
				<div>
					...
					<div>http://<span class="red">쇼핑몰도메인입력</span>/payment/kspay_return</div>
				</div>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<th class="its-th" rowspan="2">에스크로
		...
		<tr>
			...
			<td class="its-td" style="word-break:break-all;max-width:370px;">
...
<?php if($TPL_VAR["escrowMark"]){?>
						<img src="/data/icon/escrow_mark/<?php echo $TPL_VAR["escrowMark"]?>" class="webftpFormItemPreview" style="max-width:300px" style="max-width:300px;"/>
<?php }?>
					</div>
					<div class="fl pdl10">
						<div>
	...
	<div style="float:left;width:50%">
	<table width="100%" class="table_basic inputPgSetting" style="table-layout:fixed;">
		<col width="10%" /><col width="10%" /><col width="30%" />
		<tr>
		...
		<tr>
			<th class="its-th" rowspan="4" style="min-width:100px;">일반</th>
			<td class="its-td" style="min-width:120px;">세팅 정보 등록</td>
...
<?php if($TPL_mobileCardCompanyCode_1){$TPL_I1=-1;foreach($TPL_VAR["mobileCardCompanyCode"] as $TPL_V1){$TPL_I1++;?>
					<div>
					<?php echo $TPL_VAR["arrKspayCompany"][$TPL_V1]?> <?php echo $TPL_VAR["mobileCardCompanyTerms"][$TPL_I1]?> 개월
...
<?php }}?>
		...
		<tr>
			...
			<td class="its-td" valign="top" style="word-break:break-all;max-width:370px;">
				<div>인증마크가 노출되어야 하는 영역에 치환코드를 삽입하세요! 삽입 후 인증마크를 클릭하면 인증여부를 확인할 수 있습니다</div>
				<div class="webftpFormItem clearbox" style="padding-top:5px;">
					<div class="fl">
<?php if($TPL_VAR["escrowMarkMobile"]){?>
						<img src="/data/icon/escrow_mark/<?php echo $TPL_VAR["escrowMarkMobile"]?>" class="webftpFormItemPreview" style="max-width:300px" style="max-width:300px;"/>
<?php }?>
					</div>
					<div class="fl pdl10">
						<div>
...
<?php if($TPL_arrKspayCompany_1){foreach($TPL_VAR["arrKspayCompany"] as $TPL_K1=>$TPL_V1){?>
