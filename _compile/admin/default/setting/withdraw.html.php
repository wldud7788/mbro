<?php /* Template_ 2.2.6 2022/05/17 12:37:03 /www/music_brother_firstmall_kr/admin/skin/default/setting/withdraw.html 000008881 */ ?>
<!-- 회원설정 : 로그아웃/탈퇴/재가입 -->
<script type="text/javascript">
var setDefault_dormancy = function(){
<?php if(($TPL_VAR["dormancy"]=='namecheck'&&$TPL_VAR["useRealnamephone_dormancy"]!='Y'&&$TPL_VAR["useIpin_dormancy"]!='Y')){?>
		$("input[name='dormancy'][value='auto']").attr('checked','checked');
<?php }?>
};
$(document).ready(function() {
	$("input[name='dormancy'][value='<?php echo $TPL_VAR["dormancy"]?>']").attr('checked','checked');
	$("input[name='sessLimit'][value='<?php echo $TPL_VAR["sessLimit"]?>']").attr('checked','checked');
	$("input[name='modifyPW'][value='<?php echo $TPL_VAR["modifyPW"]?>']").attr('checked','checked');
	$("select[name='sessLimitMin']").val('<?php echo $TPL_VAR["sessLimitMin"]?>');
	$("select[name='modifyPWMin']").val('<?php echo $TPL_VAR["modifyPWMin"]?>');
		...
		<tr>
			...
			<td>
				<div class="resp_radio">
					<label><input type="radio" name="confirmPW" value="Y" <?php if($TPL_VAR["confirmPW"]){?>checked<?php }?>/> 사용함</label>
					<label><input type="radio" name="confirmPW" value="N" <?php if($TPL_VAR["confirmPW"]==""||$TPL_VAR["confirmPW"]=="N"){?>checked<?php }?>/> 사용 안 함</label>
				</div>
			</td>
		</tr>
		<tr>
...
<?php }else{?>
						<div class="resp_radio">
							<label><input type="radio" name="confirmPhone" class="p_chg" value="Y" <?php if($TPL_VAR["confirmPhone"]){?>checked<?php }?>/> 사용함</label>
							<label><input type="radio" name="confirmPhone" class="p_chg" value="N" <?php if($TPL_VAR["confirmPhone"]==""||$TPL_VAR["confirmPhone"]=="N"){?>checked<?php }?>/> 사용 안 함</label>
						</div>
<?php }?>
					</li>				
				</ul>
...
<?php if($TPL_VAR["sms_st"]==='Y'){?>
		<tr class="sms-send-tr" style="display:<?php if($TPL_VAR["confirmPhone"]==="Y"){?>table-row<?php }else{?>none<?php }?>;">
			<th>SMS 발송 문자</th>
			<td>						
				<input type="text" name="confirmsendmsg" class="line p_chg" value="<?php if($TPL_VAR["confirmsendmsg"]){?><?php echo $TPL_VAR["confirmsendmsg"]?><?php }else{?>&#123;shopname&#125; 인증번호는 &#123;phonecertify&#125; 입니다.<?php }?>" size="70" title="&#123;shopname&#125; 인증번호는 &#123;phonecertify&#125; 입니다." <?php if($TPL_VAR["confirmPhone"]==""||$TPL_VAR["confirmPhone"]=="N"){?>disabled="disabled"<?php }?> />
		...
		<tr>
			...
			<td>
				<div class="resp_radio">
					...
					<label><input type="radio" name="dormancy" value="namecheck" <?php if($TPL_VAR["useRealnamephone_dormancy"]!='Y'&&$TPL_VAR["useIpin_dormancy"]!='Y'){?>disabled<?php }?>/> 본인 인증 시 해제</label>
