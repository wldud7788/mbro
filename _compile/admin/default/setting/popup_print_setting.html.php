<?php /* Template_ 2.2.6 2020/12/01 09:20:51 /www/music_brother_firstmall_kr/admin/skin/default/setting/popup_print_setting.html 000013361 */ ?>
...
<ul>
	<li>
		<span class="question fx12 black">▶ 주문내역서에 주문번호를 바코드로 출력하시겠습니까?</span>
		<div class="information fx11 gray">주문 검색창에서 바코드를 스캔하면 해당 주문건의 출고화면으로 바로 이동하여 출고처리가 편리해집니다.</div>
		<div class="input">
			<label><input type="radio" name="orderPrintOrderBarcode" value="1" <?php if($TPL_VAR["order_barcode"]){?>checked<?php }?> /> 예 ( <?php echo $TPL_VAR["use_code_order"]["code_name"]?> )</label>
			<label><input type="radio" name="orderPrintOrderBarcode" value="" <?php if(!$TPL_VAR["order_barcode"]){?>checked<?php }?> /> 아니오</label>
		</div>
	</li>
	<li>
...
<?php }?>
	...
	<li>
		<span class="question fx12 black">▶ 주문내역서에 주문상품의 상품코드를 바코드로 출력하시겠습니까?</span>
		<div class="information fx11 gray">해당 주문의 출고처리화면에서 실제 상품의 바코드를 스캔하면 주문상품이 맞는지 검증하여 <br />
		오배송 없이 정확하게 출고가 가능합니다. 해당 상품의 바코드를 계속 스캔하면 출고수량이 +1씩 증가합니다.</div>
		<div class="input">
			<label><input type="radio" name="orderPrintGoodsBarcode" value="1" <?php if($TPL_VAR["order_goods_barcode"]){?>checked<?php }?> /> 예 ( <?php echo $TPL_VAR["use_code"]["code_name"]?> )</label> &nbsp;
			<label><input type="radio" name="orderPrintGoodsBarcode" value="" <?php if(!$TPL_VAR["order_goods_barcode"]){?>checked<?php }?> /> 아니오</label>
		</div>
	</li>
	<li>
...
<ul>
	<li>
		<span class="question fx12 black">▶ 출고내역서에 출고번호를 바코드로 출력하시겠습니까?</span>
		<div class="information fx11 gray">출고 검색창에서 바코드를 스캔하면 해당 출고건의 출고화면으로 바로 이동하여 출고처리가 편리해집니다.</div>
		<div class="input">
			<label><input type="radio" name="exportPrintExportcodeBarcode" value="1" <?php if($TPL_VAR["export_code_barcode"]){?>checked<?php }?> /> 예 ( <?php echo $TPL_VAR["use_code_order"]["code_name"]?> )</label> &nbsp;
			<label><input type="radio" name="exportPrintExportcodeBarcode" value="" <?php if(!$TPL_VAR["export_code_barcode"]){?>checked<?php }?> /> 아니오</label>
		</div>
	</li>
	<li>
		<span class="question fx12 black">▶
...
<?php }?>
	...
	<li>
		<span class="question fx12 black">▶ 출고내역서에 출고상품의 상품코드를 바코드로 출력하시겠습니까?</span>
		<div class="information fx11 gray">해당 출고의 출고처리화면에서 실제 상품의 바코드를 스캔하면 출고상품이 맞는지 검증하여 <br />
		오배송 없이 정확하게 출고가 가능합니다. 해당 상품의 바코드를 계속 스캔하면 출고수량이 +1씩 증가합니다.</div>
		<div class="input">
			<label><input type="radio" name="exportPrintGoodsBarcode" value="1" <?php if($TPL_VAR["export_goods_barcode"]){?>checked<?php }?> /> 예 ( <?php echo $TPL_VAR["use_code"]["code_name"]?> )</label> &nbsp; 
			<label><input type="radio" name="exportPrintGoodsBarcode" value="" <?php if(!$TPL_VAR["export_goods_barcode"]){?>checked<?php }?> /> 아니오</label>
		</div>
	</li>
	<li>
