<?php /* Template_ 2.2.6 2022/05/17 12:36:58 /www/music_brother_firstmall_kr/admin/skin/default/setting/manager_reg.html 000088604 */ 
...
<?php }?>
					...
					if(auth_flag || auth_limit){
						...
						$('.auth, .authboard').attr("disabled", true);
					}
...
<?php }?>
					...
					<th>
						비밀번호 설정
<?php if(!$TPL_VAR["manager_seq"]){?><span class="tooltip_btn" onClick="showTooltip(this, '/admin/tooltip/manager', '#tip6', 'sizeM')"></span><?php }?>
					</th>
					<td>
<?php if($TPL_VAR["manager_seq"]){?>
...
<?php }?>
					...
					<tr class="ip_view">
						...
						<td>
<?php if($TPL_VAR["managerInfo"]["manager_yn"]=='Y'){?>
							<table class="table_basic wauto " id="ipViewTable">
								<thead>
								<th>IP</th>
...
<?php }?>
					...
					<tr class="admin_limit_ip">
						...
						<td>
<?php if($TPL_VAR["auth"]["manager_yn"]=='Y'){?>
							<table class="table_basic wauto" id="adminIpCell">
								<colgroup>
									<col width="300px" /><col width="80px" />
								</colgroup>
								<thead>
								<th>IP</th>
			...
			<div class="contents_dvs">
				...
				<table class="table_basic thl">
					<tr>
						...
						<td>
							<table id="cerfify_manager" class="table_basic tdc wx900">
								<colgroup>
...
<?php if($TPL_I1== 0){?>
					<tr>
						<th class="its-th" rowspan="4">주문</th>
						<td class="its-td">
							<!-- 주문 보기 -->
							<label class="resp_checkbox"><input type="checkbox" name="<?php echo $TPL_V1["codecd"]?>" value="Y" class="auth"/> <?php echo $TPL_V1["value"]?></label>
<?php }?>
...
<?php }}?>
						...
						<td class="its-td">
<?php if(is_array($TPL_R1=code_load('auth_ifdo_marketing'))&&!empty($TPL_R1)){foreach($TPL_R1 as $TPL_V1){?>
							<!-- IFDO 마케팅 설정 -->
							<label class="resp_checkbox"><input type="checkbox" name="<?php echo $TPL_V1["codecd"]?>" value="Y" class="auth"/> <?php echo $TPL_V1["value"]?></label>
<?php }}?>
						</td>
