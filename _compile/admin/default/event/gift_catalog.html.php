<?php /* Template_ 2.2.6 2022/05/30 15:46:07 /www/music_brother_firstmall_kr/admin/skin/default/event/gift_catalog.html 000012758 */ 
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		gSearchForm.init({'pageid':'gift_catalog','search_mode':'<?php echo $TPL_VAR["sc"]["search_mode"]?>','select_date':'<?php echo $TPL_VAR["sc"]["select_date"]?>','displaySort':arrSort});
		...
		//var src_provider_seq = '<?php echo $TPL_VAR["sc"]["provider_seq"]?>';
...
<?php }?>
		...
		<ul class="page-buttons-right">
			<li><button onclick="location.href='/admin/event/gift_regist'" class="resp_btn active2 size_L">이벤트 등록</button></li>			
		</ul>
	</div>
</div>
<!-- 페이지 타이틀 바 : 끝 -->
		...
		<tr>
			...
			<td>	
				<span style="display:inline-block">
					...
					<input type="hidden" class="provider_seq" name="provider_seq" value="<?php echo $TPL_VAR["sc"]["provider_seq"]?>" relation="ship_grp" />
				</span>
...
<?php if($TPL_record_1){foreach($TPL_VAR["record"] as $TPL_V1){?>
			<!-- 리스트타이틀(이벤트상태 및 버튼) : 시작 -->
				...
				<td><?php echo $TPL_V1["_no"]?></td>
					...
					<td class="left"><a href='/admin/event/gift_regist?event_seq=<?php echo $TPL_V1["gift_seq"]?>' class='resp_btn_txt v2'><?php echo $TPL_V1["title"]?></a></td>
<?php if(serviceLimit('H_AD')){?>				
