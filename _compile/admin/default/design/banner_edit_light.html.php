<?php /* Template_ 2.2.6 2022/05/17 12:31:33 /www/music_brother_firstmall_kr/admin/skin/default/design/banner_edit_light.html 000023290 */ 
$TPL_banner_styles_1=empty($TPL_VAR["banner_styles"])||!is_array($TPL_VAR["banner_styles"])?0:count($TPL_VAR["banner_styles"]);?>
<?php $this->print_("layout_header_popup",$TPL_SCP,1);?>


<link rel="stylesheet" type="text/css" href="/app/javascript/plugin/slick/slick.css"><!-- 반응형 슬라이드 -->
<link rel="stylesheet" type="text/css" href="/app/javascript/plugin/jquploadify/uploadify.css" />
<script src="/app/javascript/plugin/slick/slick.min.js?v=<?php echo date('YmdHis')?>"></script><!-- slick slider( 반응형 ) -->
...
<?php if(!$_GET["popup"]){?>
<?php if($TPL_VAR["template_path"]&&$TPL_VAR["banner_seq"]){?>
			parent.DM_window_title_set("left","<a href=\"javascript:;\" onmouseup=\"DM_window_sourceeditor('<?php echo $TPL_VAR["template_path"]?>','\{=showDesignBanner(<?php echo $TPL_VAR["banner_seq"]?>)\}')\">◀ 슬라이드 배너 영역의 HTML소스보기</a>");
...
<?php }?>
		...
		// 생성시 기본 이미지입력폼
<?php if($TPL_VAR["banner_seq"]){?>
		banner_setting_load(false,'<?php echo $TPL_VAR["banner_seq"]?>');
<?php }else{?>
		banner_item_add();
...
<?php }?>
					...
					if(res.images && res.images.length>0){
						for(var i=0;i<res.images.length;i++){
							...
							if(is_sample){
								...
								obj.find("img.webftpFormItemPreview").attr('src','/data/skin/'+design_working_skin+'/'+res.images[i].image+'?'+Math.floor(Math.random()*1000000000)).show();
								obj.find("input[name='image[]']").val('/data/skin/'+design_working_skin+'/'+res.images[i].image);
							}
	...
	function banner_item_html(){
		...
		html += '		<span class="orange" style="display:inline-block; margin-top:4px;">등록 이미지 (<span class="webftpFormItemPreviewSize"></span>)</span>';
		html += '	</td>';
...
<input type="hidden" name="banner_seq" value="<?php echo $TPL_VAR["banner_seq"]?>" />
...
<?php }?>
			...
			<td class="dsts-td left">
				<select name="style" <?php if($TPL_VAR["banner_seq"]){?>onmousedown="alert('스타일은 변경이 불가능합니다.\n해당 영역을 삭제 후 새로 등록해주세요.');" style="background-color:#b7b7b7;"<?php }?>>
<?php if($TPL_banner_styles_1){foreach($TPL_VAR["banner_styles"] as $TPL_K1=>$TPL_V1){?>
					<option value="<?php echo $TPL_K1?>" platform="<?php echo $TPL_V1["platform"]?>"><?php echo $TPL_V1["name"]?></option>
<?php }}?>
				</select>
			</td>
		</tr>
...
<?php if($TPL_VAR["template_path"]){?>
			<span class="btn large"><input type="button" value="목록" onclick="parent.DM_window_banner_insert('<?php echo $TPL_VAR["template_path"]?>')"/></span>
<?php }?>
