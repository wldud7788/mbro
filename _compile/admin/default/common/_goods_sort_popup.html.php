<?php /* Template_ 2.2.6 2020/12/01 09:20:51 /www/music_brother_firstmall_kr/admin/skin/default/common/_goods_sort_popup.html 000008773 */ ?>
<?

 $TPL_VAR["tmpImageWidth"] = floor(100 /  $TPL_VAR["count_w"]) . '%';

?>
<script type="text/javascript" src="/app/javascript/plugin/jquery_pagination/jquery.pager.js" charset="utf8"></script>
<script>
var now_page = 1;
var kind = '<?php echo $_GET["kind"]?>';
var code = '<?php echo $_GET["code"]?>';
var count_w = '<?php echo $_GET["count_w"]?>';
var count_h = '<?php echo $_GET["count_h"]?>';
var mobile_setting = '<?php echo $_GET["mobile_setting"]?>';
...
function set_sort_list(result){
	...
	for(var i=0;i<result.record.length;i++){
		...
		html += "<img src='"+result.record[i].image+"?"+nowTime+"' onerror=\"this.src='/admin/skin/default/images/common/noimage_list.gif'\">";
		html += "<div><span class='desc'>"+ result.record[i].sort+" ["+result.record[i].goods_seq+"]<br />"+result.record[i].goods_status_char+"<br />"+result.record[i].goods_name_chars+"</span></div>";
		html += "</li>";
	}
...
.goods-sort-list li { display: flex; flex-direction: column; box-sizing: border-box; width: <?php echo $TPL_VAR["tmpImageWidth"]?>; overflow:hidden; border:1px solid #fff; background-color:#fff; cursor:move; text-align:center}
.goods-sort-list li img {width: 100%;}
.goods-sort-list li div {text-align:center; padding-top:5px; margin-bottom:5px; font-size:10px;}
