<?php /* Template_ 2.2.6 2022/05/25 11:33:36 /www/music_brother_firstmall_kr/admin/skin/default/export/batch_status.html 000084311 */  $this->include_("scmSelectWarehouse");
...
table.table-export-info tr.close-tr td.option { border-bottom:1px solid #000 !important; }

table.store-stock tr:first-child th {border-top:2px solid black !important;}
table.store-stock tr:nth-last-child(1) td {	border-bottom:2px solid black !important;}
table.store-stock tr td:first-child {border-left:2px solid black !important;}
table.store-stock tr th:first-child {border-left:2px solid black !important;}
table.store-stock tr td:nth-last-child(2) {	border-left:2px solid red !important;}
table.store-stock tr td:nth-last-child(1) {	border-right:2px solid red !important;}
table.store-stock tr:last-child td:nth-last-child(2) {border-bottom:2px solid red !important;}
table.store-stock tr:last-child td:nth-last-child(1) {border-bottom:2px solid red !important;}
table.store-stock tr:first-child th:nth-last-child(2) {border-top:2px solid red !important;border-left:2px solid red !important;}
table.store-stock tr:first-child th:nth-last-child(1) {border-top:2px solid red !important;	border-right:2px solid red !important;}
table.store-stock tr th,td {line-height:120% !important;}
table.list-goods-info tr td {border:0px;}
...
<div class="search-form-container">
			...
			<tr>
				<td width="415">
					...
					<tr>
						<td class="sfk-td-txt">
							<div class="relative">
								<input type="text" name="keyword" id="search_keyword" value="<?php echo $_GET["keyword"]?>" title="출고번호,주문번호,아이디,주문자,수령자,입금자,이메일,연락처,휴대폰,상품명,상품번호,상품코드" />
				...
				<td align="right">
					...
					<button type="button" id="get_default_button" value="기본검색적용" name="get_default_button" onclick="set_default_search_form();"></button>
					<button type="button" id="btn-reset" title="검색초기화" onclick="reset_search_form();"></button>
	...
	<tr>
		<td>
			<table class="sf-option-table" border="0" style="table-layout:fixed;">
			<colgroup>
				<col width="110" />
				<col width="*" />
				<col width="170" />
...
<?php if(serviceLimit('H_AD')){?>
			<tr>
				<th class="pdr10 right">배송책임</th>
				<td colspan="3">
					<div class="ui-widget pdl10" style="font-size:12px;" id="provider_select">
...
<?php }}?>
						...
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<input type="text" name="provider_name" value="" style="width:200px" readonly />
						&nbsp;&nbsp;
					</div>
				</td>
			<tr>
<?php }?>
			<tr>
				...
				<td width="*">
					<input type="text" name="regist_date[]" value="<?php echo $_GET["start_search_date"]?>" class="datepicker line"  maxlength="10" size="10" />
					&nbsp;<span class="gray">-</span>&nbsp;
					<input type="text" name="regist_date[]" value="<?php echo $_GET["end_search_date"]?>" class="datepicker line" maxlength="10" size="10" />
					<span class="btn small"><input type="button" value="오늘" onclick="set_date_export('<?php echo date('Y-m-d')?>','<?php echo date('Y-m-d')?>');" style="border:0;" /></span>
					<span class="btn small"><input type="button" value="3일간" onclick="set_date_export('<?php echo date('Y-m-d',strtotime('-3 day'))?>','<?php echo date('Y-m-d')?>');" style="border:0;" /></span>
					<span class="btn small"><input type="button" value="일주일" onclick="set_date_export('<?php echo date('Y-m-d',strtotime('-7 day'))?>','<?php echo date('Y-m-d')?>');" style="border:0;" /></span>
					<span class="btn small"><input type="button" value="1개월" onclick="set_date_export('<?php echo date('Y-m-d',strtotime('-1 month'))?>','<?php echo date('Y-m-d')?>');" style="border:0;" /></span>
					<span class="btn small"><input type="button" value="3개월" onclick="set_date_export('<?php echo date('Y-m-d',strtotime('-3 month'))?>','<?php echo date('Y-m-d')?>');" style="border:0;" /></span>
					<span class="btn small"><input type="button" value="전체" onclick="set_date_export('','');" style="border:0;"/></span>
				</td>
				<td rowspan="2">
					<div class="selectbox_multi">
						<div class="cont">
							...
							<div class="list" style="height:70px;">
							<ul>
<?php if($TPL_ship_set_code_1){foreach($TPL_VAR["ship_set_code"] as $TPL_K1=>$TPL_V1){?>
								<li><label><input type="checkbox" name="search_shipping_method[]" value="<?php echo $TPL_K1?>" title="<?php echo $TPL_V1?>" <?php if($_GET['shipping_method'][$TPL_K1]==$TPL_K1){?>checked<?php }?> default_none> <?php echo $TPL_V1?></label></li>
<?php }}?>
								<li><label><input type="checkbox" name="search_shipping_method[]" value="coupon" title="문자/이메일" <?php if($_GET['shipping_method']['coupon']=='coupon'){?>checked<?php }?> default_none> 문자/이메일</label></li>
							</ul>
							</div>
						</div>
...
<?php if($TPL_VAR["connectorUse"]==true&&$TPL_VAR["isExportedList"]==true){?>
					<div class="selectbox_multi">
						<div class="cont multiselect">
							<h2><label><input type="checkbox" name="allselectMarkets" class="allSelectDrop" default_none value='y' <?php if(count($TPL_VAR["marketList"])==count($_GET["selectMarkets"])){?>checked<?php }?>> <span class="allselectMarkets">오픈 마켓 전체</span></label></h2>
							<div class="list">
								<ul>
...
<?php if(is_array($TPL_R1=config_load('export_status'))&&!empty($TPL_R1)){foreach($TPL_R1 as $TPL_K1=>$TPL_V1){?>
					<label class="search_label" style="display:inline-block;padding:3px 3px 3px 0px;width:90px;">
						...
						<span class="icon-order-step-<?php echo $TPL_K1?>"><?php echo $TPL_V1?></span></label>
					</label>
<?php }}?>
...
<?php if($TPL_VAR["npay_use"]&&false){?>
					<label style="margin-left:20px;display:inline-block;height:26px;vertical-align:middle;line-height:26px;">
						<input type="checkbox" name="search_npay_order" value='y' <?php if($_GET["search_npay_order"]=='y'){?>checked<?php }?> align="middle"> Npay 주문건</label>
<?php }?>
				</td>
			</tr>
			<tr>
				<th class="pdr10 right">택배사</th>
				<td>
...
<?php if($TPL_K1==$_GET["src_shipping_delivery"]){?>
...
<?php if($TPL_V1["company"]==$_GET["src_shipping_delivery"]){?>
...
<?php }?>
						...
						<label><input type="checkbox" name="none_search_delivery_number"  value="1" class="search_delivery" <?php if($_GET["none_search_delivery_number"]){?>checked<?php }?> /> 없음</label>
					</div>
				</td>
			</tr>
			</table>
...
<div align="center">
	<div style="position: relative;width:100px;height:5px;">
		<div style="position: absolute;z-index:2;top:-5px;" id="btn-close-search" <?php if($_GET["seq"]){?>class="hide"<?php }?>><a href="javascript:close_search_form();"><img src="/admin/skin/default/images/common/search_close.gif" /></a></div>
		<div style="position: absolute;z-index:2;top:-5px;" id="btn-open-search" <?php if(!$_GET["seq"]){?>class="hide"<?php }?>><a href="javascript:open_search_form();"><img src="/admin/skin/default/images/common/search_open.gif" /></a></div>
	</div>
</div>
...
<?php if($TPL_VAR["hasMarketOrders"]==false&&$TPL_V2["export_status"]!='y'&&strpos($TPL_V2["linkage_mall_order_id"],'ex-')===false){?>
						<input type="checkbox" name="export_code[]" disabled="disabled" />	
<?php }else{?>
<?php if($TPL_VAR["hasMarketOrders"]!=false&&strpos($TPL_V2["linkage_mall_order_id"],'ex-')!==false){?>
								<input type="checkbox" name="export_code[]" disabled="disabled" />
<?php }else{?>
...
<?php if(is_array($TPL_R3=$TPL_V2["bundle_order_list"])&&!empty($TPL_R3)){foreach($TPL_R3 as $TPL_V3){?>
									...
									<a href="javascript:printOrderView('<?php echo $TPL_V3?>')"><span class="icon-print-order"></span></a>
<?php }}?>
<?php }else{?>
...
<?php }?>
					<tr>
						<td>
							...
							<a href="javascript:printExportView('<?php echo $TPL_V2["order_seq"]?>','<?php echo $TPL_V2["export_code"]?>')"><span class="icon-print-export"></span></a>
<?php if($TPL_V2["is_bundle_export"]=='Y'){?><span class="bold red">[합포장]</span><?php }?>
<?php if(serviceLimit('H_AD')){?>
							<div class="bold">
...
<?php }?>
						...
						<td width="25"></td>
						<td>
<?php if($TPL_V2["shipping_method"]&&$TPL_V2["shipping_method"]!='coupon'&&$TPL_V2["shipping_method"]!='direct_store'){?>
							<span class="addr">
...
<?php }elseif($TPL_V2["shipping_method"]=='direct_store'){?>
...
<?php if($TPL_V2["shipping_method"]&&$TPL_V2["shipping_method"]!='coupon'){?>
					<tr>
						<td></td>
						<td>
...
<?php }elseif($_GET["status"]=='55'){?>
<?php if($TPL_VAR["hasMarketOrders"]==true&&strpos($TPL_V2["linkage_mall_order_id"],'ex-')===false){?>
						<span class="btn small cyanblue" >
...
<?php if($TPL_I1== 0&&$TPL_I2== 0){?>
		<tr>
			<th>주문상품</th>
			<th>
				<div <?php if($_GET["status"]!= 45){?>class="hide"<?php }?>>
...
<?php }?>
			...
			<td class="info option" align="center"><span class="total_stock_<?php echo $TPL_V2["export_code"]?>"><?php echo number_format($TPL_V2["tot_stock"])?></span></td>
...
<?php if($TPL_VAR["gf_config"]["goodsflow_step"]=='1'&&$TPL_VAR["gf_config"]["gf_use"]=='Y'){?>
				<tr class="gf_btn_area <?php if($TPL_V2["shipping_provider_seq"]!=$TPL_VAR["gf_config"]["provider_seq"]||$TPL_VAR["gf_config"]["gf_deliveryCode"]!=$TPL_V2["delivery_company_code"]||!preg_match('/delivery|postpaid/',$TPL_V2["shipping_method"])){?>hide<?php }?>">
					<td class="info pdl5">
						<!-- #23611 ycg 2018-12-07 shoplinker n goodsflow linkage
...
<?php if($TPL_VAR["scm_cfg"]['use']=='Y'&&$TPL_V2["provider_seq"]== 1&&$TPL_V2["package_yn"]!='y'){?>
				<input type="hidden" name="optioninfo[<?php echo $TPL_V2["export_code"]?>][option][<?php echo $TPL_V2["goods_option_seq"]?>]" class="optioninfo" value="<?php echo $TPL_V2["goods_seq"]?>option<?php echo $TPL_V2["goods_option_seq"]?>" />
				<input type="hidden" name="whSupplyPrice[<?php echo $TPL_V2["export_code"]?>][option][<?php echo $TPL_V2["goods_option_seq"]?>]" class="wh_supply_price" value="<?php echo $TPL_V2["scm_supply_price"]?>" />
				<input type="hidden" name="goodscode[<?php echo $TPL_V2["export_code"]?>][option][<?php echo $TPL_V2["goods_option_seq"]?>]" value="<?php echo $TPL_V2["opt_goods_code"]?>" />
<?php }?>
...
<?php if($TPL_V2["option1"]){?>
						<div class="goods_option">
							<img src="/admin/skin/default/images/common/icon_option.gif" />
<?php if($TPL_V2["option1"]){?><?php if($TPL_V2["title1"]){?><?php echo $TPL_V2["title1"]?>:<?php }?><?php echo $TPL_V2["option1"]?><?php }?>
...
<?php if($TPL_V2["package_yn"]=='y'){?>
						실제상품▼
<?php }else{?>
						<span class="stock"><?php echo number_format($TPL_V2["stock"])?></span>
<?php }?>
<?php }else{?>
				-
<?php }?>
...
<?php if($TPL_VAR["scm_cfg"]['use']=='Y'&&$TPL_V2["shipping_provider_seq"]== 1){?>
				...
				<input type="hidden" name="goodscode[<?php echo $TPL_V2["export_code"]?>][option][<?php echo $TPL_V3["option_seq"]?>]" value="<?php echo $TPL_V3["opt_goods_code"]?>" />
<?php }?>
