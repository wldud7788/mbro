<?php /* Template_ 2.2.6 2022/05/31 15:46:45 /www/music_brother_firstmall_kr/admin/skin/default/board/write.html 000017094 */ 
...
<?php $this->print_("noticehidden",$TPL_SCP,1);?>
			...
			<tr>
				<th>내용</th>
				<td >
					<textarea name="contents" id="contents" class="daumeditor required" style='width:95%; height:300px;'><?php echo $TPL_VAR["contents"]?></textarea>
				</td>
			</tr>
			<!-- 유튜브 영상 보이는 페이지 by 김혜진 -->
<?php if($_GET["id"]=='video'){?>
			<tr>
				<th>유튜브 영상코드</th>
				<td>
					<input type="text" id="video_url" name="video_url" value="<?php echo $TPL_VAR["youtube"]?>">
					<button type="button" id="youtube_url" onclick="check_url()">미리보기</button>
				</td>
			</tr>
			<tr>
				<th>유튜브 영상</th>
				<td colspan="3">
					<iframe id="youtube_preview" width="560" height="315" src="https://www.youtube.com/embed/<?php echo $TPL_VAR["youtube"]?>" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
				</td>
			</tr>
<?php }?>
...
<script type="text/javascript">
	// <!--2022.02.14 유튜브 영상 등록 과정 by 김혜진 -->
	$(document).ready(function () {
		var board_id = '<?php echo $_GET["id"]?>';
		if (board_id == 'video') {
			var origin = document.getElementById("youtube_url").value;
			document.getElementById("youtube_preview").src = 'https://www.youtube.com/embed/'+origin;
		}
	});
	...
	function check_url() {
		...
		document.getElementById("youtube_preview").src = 'https://www.youtube.com/embed/'+url;
	}
