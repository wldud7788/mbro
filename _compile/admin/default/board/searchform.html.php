<?php /* Template_ 2.2.6 2022/07/15 14:55:29 /www/music_brother_firstmall_kr/admin/skin/default/board/searchform.html 000013193 */ 
...
<?php }}?>
		...
		<ul class="page-buttons-right">
			<li>
				<button type="button" name="boardwrite" id="boad_write_btn" title="<?php echo $TPL_VAR["manager"]["name"]?>" class="resp_btn active2 size_L">게시글 등록</button>
			</li>
		</ul>
...
<div id="search_container" class="search_container">
	<form name="boardsearch" id="boardsearch">
		<input type="hidden" name="id" value="<?php echo $_GET["id"]?>" defaultValue="<?php echo $_GET["id"]?>">
		...
		<input type="hidden" name="score_avg" id="score_avg" value="<?php echo $_GET["score_avg"]?>">
...
<?php if($_GET["id"]==='goods_qna'||$_GET["id"]==='goods_review'||$_GET["id"]==='bulkorder'){?>
...
<?php if($_GET["id"]!='faq'){?>
						<span class="resp_checkbox">
							<label>
								<input type="checkbox" name="hidden" id="searchhidden" value="1"<?php if($_GET["hidden"]=='1'){?> checked<?php }?>>
								<span>비밀글 제외</span>
							</label>
							<label>
								<input type="checkbox" name="display" id="searchdisplay" value="1"<?php if($_GET["display"]=='1'){?> checked<?php }?>>
								<span>삭제글 제외</span>
							</label>
						</span>
<?php }?>
			...
			<tr>
				...
				<td>
					<span class="resp_radio">
						<label>
							<input type="radio" name="searchreply" value<?php if(!$_GET["searchreply"]){?> checked<?php }?>>
							<span>전체</span>
						</label>
						<label>
							<input type="radio" name="searchreply" value="y"<?php if($_GET["searchreply"]=='y'){?> checked<?php }?>>
							<span>답변 대기</span>
						</label>
						<label>
							<input type="radio" name="searchreply" value="n"<?php if($_GET["searchreply"]=='n'){?> checked<?php }?>>
							<span>답변 완료</span>
						</label>
					</span>
				</td>
...
<?php if($TPL_boardmanagercopylist_1){foreach($TPL_VAR["boardmanagercopylist"] as $TPL_V1){?>
								<option value="<?php echo $TPL_V1["id"]?>"><?php echo getstrcut($TPL_V1["name"], 10)?> (<?php echo number_format($TPL_V1["totalnum"])?>)</option>
<?php }}?>
							</select>
						</td>
					</tr>
