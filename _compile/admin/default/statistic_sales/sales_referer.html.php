<?php /* Template_ 2.2.6 2022/05/17 12:37:15 /www/music_brother_firstmall_kr/admin/skin/default/statistic_sales/sales_referer.html 000012075 */ ?>
	...
	<div class='slc-body-wrap'>
		<div class="slc-body">					
			<div class="chartBlock">				
				<div class="item-title">판매 통계 - 유입경로</div>				
...
<?php }}?>
				...
				<div class="sub_title mt30">판매(결제) 횟수</span></div>				
				<div id="cntChart" style="margin:auto; height:250px; width:1000px;"></div>				
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<td align="right" valign="bottom" class="pdt5 pdb10">
							...
							<span class="btn small"><input type="button" value="엑셀출력" onclick="divExcelDownload('<?php if($TPL_VAR["sc"]["dateSel_type"]=='daily'){?>일별<?php }else{?>월별<?php }?>_매출유입경로통계_판매건수','#referer_cnt_table')" /></span>
						</td>
					</tr>
				</table>	
				
				<div id="referer_cnt_table">
<?php $this->print_("referer_cnt_table",$TPL_SCP,1);?>
			...
			<div class="chartBlock">
				...
				<div id="priceChart" style="margin:auto; height:250px; width:1000px;"></div>
				<br style="line-height:20px" />
			</div>
			<div class="tableBlock">
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<td align="right" valign="bottom" class="pdt5 pdb10">
							<span class="btn small orange"><input type="button" value="안내) 네이버광고 유입경로"  class="guide_naver_referer" /></span>
							<span class="btn small"><input type="button" value="엑셀출력" onclick="divExcelDownload('<?php if($TPL_VAR["sc"]["dateSel_type"]=='daily'){?>일별<?php }else{?>월별<?php }?>_매출유입경로통계_판매금액','#referer_price_table')" /></span>
						</td>
					</tr>
				</table>	
				
				<div id="referer_price_table">
<?php $this->print_("referer_price_table",$TPL_SCP,1);?>
...
<div id="layer_guide_naver_referer" class="fx12 hide">
	<div>
		네이버 광고를 통한 유입 분석(방문/가입/판매)이 가능하기 위해서는 네이버 광고 시 반드시 추적 URL을 설정해 주셔야 합니다.<br />
...
<?php if(is_array($TPL_R1=$TPL_VAR["dataForChart"]['cnt'])&&!empty($TPL_R1)){foreach($TPL_R1 as $TPL_K1=>$TPL_V1){?>
...
<?php }}?>
	...
	function makePlotPrice(){
		...
		var listData	= new Object();
<?php if(is_array($TPL_R1=$TPL_VAR["dataForChart"]['price'])&&!empty($TPL_R1)){foreach($TPL_R1 as $TPL_K1=>$TPL_V1){?>
		listData['<?php echo $TPL_K1?>']	= <?php echo json_encode($TPL_V1)?>;
<?php }}?>
