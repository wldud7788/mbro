<?php /* Template_ 2.2.6 2022/09/15 17:42:15 /www/music_brother_firstmall_kr/selleradmin/skin/default/setting/delivery_company.html 000052426 */ 
...
$(document).ready(function() {
	...
	$(".chk_duple").bind('click', function(){
		...
		// 중복체크 실행
		$.ajax({
			'url' : '/admin/setting_process/epost_duple_chk',
			'type' : 'post',
	...
	$("#set_epostuse").bind('click', function(){
		...
		$.ajax({
			'url' : '/admin/setting_process/epost_use_save',
			'type' : 'post',
	...
	$("#epost_cancel").bind('click', function(){
		$.ajax({
			'url' : '/admin/setting_process/epost_cancel',
			'type' : 'post',
	...
	$("#invoiceSetting").bind('click', function(){
		openDialog("택배 업무 자동화 연동 - 롯데택배", "invoiceSettingPopup", {"width":900});
	...
	$("#gf_log").bind('click', function(){
		openDialog("굿스플로 사용현황 <span class='desc'>&nbsp;</span>", "goodsflow_log_area", {"width":"1000","height":"720"}); 
	});
	...
	$("#goodsflow_use_setting").bind('click', function(){
		openDialog("입점사 이용 여부", "goodsflow_use_area", {"width":550,"close":function(){
...
function set_delivery_company(){
	openDialog('택배사', 'delivery_set_lay', {'width':'550','height':'650'});
}
...
<div id="page-title-bar-area">
	<div id="page-title-bar">
		...
		<div class="page-buttons-right">
			<button type="button" onclick="set_delivery_company();" class="resp_btn active size_L">택배사 설정</button>
		</div>
...
<div class="contents_dvs v2">
	...
	<table class="table_basic v7 v10 tdc">
	<colgroup>
			<col width="5%" />
			<col />
			<col width="8%" />
...
<?php if(($TPL_K1=='code22'&&$TPL_VAR["config_goodsflow"]["setting"]['goodsflow_msg'])||($TPL_K1=='code23'&&$TPL_VAR["config_epost"]["status_msg"]&&$TPL_VAR["config_epost"]["epost_use"]!='N')||($TPL_K1=='code24'&&$TPL_VAR["config_invoice"]["hlc"]["use"]=='1')){?>
	<tr>
<?php }else{?>
	<tr>
<?php }?>
...
<?php }?>
		...
		<td class="left"><?php echo $TPL_V1["company"]?></td>
...
<?php }?>
		...
		<td class="left ">굿스플로 업무 자동화</td>
		<td>
<?php if($TPL_VAR["config_system"]["goodsflow_use"]!= 1){?>
			<button type="button" class="resp_btn v2">연동불가</button>
<?php }elseif($TPL_VAR["config_goodsflow"]["setting"]['gf_use']!='Y'){?>
			<button type="button" class="resp_btn v2" onclick="openDialogAlert('사용을 원하시면 본사에 문의하시기 바랍니다.', 400, 160);">연동불가</button>
<?php }else{?>
			<button type="button" id="goodsflowSetting" class="resp_btn v2">설정</button>					
<?php }?>
		</td>
		<td class="left">
...
<?php if($TPL_VAR["config_goodsflow"]["setting"]['goodsflow_step']=='1'){?>
			<input type="button" class="resp_btn" <?php if($TPL_VAR["isdemo"]["isdemo"]){?> <?php echo $TPL_VAR["isdemo"]["isdemojs1"]?> <?php }else{?> id="gf_charge" <?php }?> value="충전" />
<?php }else{?>
			<input type="button" class="resp_btn" value="충전" onclick="openDialogAlert('연동 완료 후 충전 가능합니다.',400,140);" />
<?php }?>
<?php }?>
			<input type="button" id="gf_log" class="resp_btn" value="사용현황" />
		</td>
...
<?php }?>
		...
		<td class="left">우체국택배 업무 자동화</td>
		<td>
			<button type="button" id="epostSetting" class="resp_btn v2">설정</button>
		</td>
		<td class="left">
...
<?php }?>
		...
		<td class="left">롯데택배 업무 자동화</td>
		<td>
			<button type="button" id="invoiceSetting" class="resp_btn v2">설정</button>
		</td>
		<td class="left">
<?php if($TPL_VAR["config_invoice"]["hlc"]["use"]=='1'){?>
				이용중
...
<div id="delivery_set_lay" class="hide">
	...
	<tr>
		<td width="40%">
			<select name="org_delivery_company[]" multiple="multiple" style="width:100%;height:430px !important;">
...
<?php }}?>
		...
		<td width="40%">
			<select name="selected_delivery_company[]" id="selected_delivery_company" multiple="multiple"  style="width:100%;height:430px !important;">
...
<div id="invoiceSettingPopup" style="display:none">
	...
	<div id="invoiceSettingAuthContainer">
		...
		<tr>
			...
			<td>
				<div>
					<input type="text" class="invoice_auth_code line" name="auth_code[hlc]" value="<?php echo $TPL_VAR["config_invoice"]["hlc"]["auth_code"]?>" auth_code="<?php echo $TPL_VAR["config_invoice"]["hlc"]["auth_code"]?>" />
					<button type="button" class="resp_btn v2" onclick="hlc_auth()" >인증</button>
		...
		<tr>
			<th>
				운송장 프린트 세팅
				<div class="gabia-pannel" code="invoice_print_setting_guide">프린트 세팅 가이드</div>
			</th>
			<td>
	...
	<div class="footer">
		<input type="submit" class="resp_btn active size_XL" value="확인" />
...
<form name="epostSettingForm" action="/admin/setting_process/epost_setting" target="actionFrame" method="post" class="hp100">
...
<?php }?>
		...
		<table class="table_basic thl mt5" id="epost_form">
		<colgroup>
		<col width="18%"/>
		<col width="32%"/>
		<col width="18%"/>
...
<?php if($TPL_VAR["config_epost"]["requestkey"]&&$TPL_VAR["config_epost"]["status"]!='7'){?>
				<input type="button" id="set_epostuse" class="btn_resp b_gray" value="저장" />
<?php }?>
...
<?php }?>
		...
		<tr>
			<th>우체국 아이디<span class="required_chk"/></th>
			<td>
<?php if($TPL_VAR["config_epost"]["status"]=='9'){?>
				*******
<?php }else{?>
...
<?php }?>
			...
			<th>우체국 비밀번호<span class="required_chk"/></th>
			<td>
<?php if($TPL_VAR["config_epost"]["status"]=='9'){?>
				*******
<?php }else{?>
...
<?php }?>
		...
		<tr>
			<th>우체국 고객번호<span class="required_chk"/></th>
			<td>
<?php if($TPL_VAR["config_epost"]["status"]=='9'){?>
...
<?php }else{?>
				...
				<input type="button" class="chk_duple resp_btn" chk_type="epost_num" value="중복확인"/>
				<span class="red duple_txt hide">사용가능</span>
<?php }?>
			</td>
			<th>우체국 승인번호<span class="required_chk"/></th>
			<td>
<?php if($TPL_VAR["config_epost"]["status"]=='9'){?>
...
<?php }else{?>
				...
				<input type="button" class="chk_duple resp_btn" chk_type="epost_auth_code[]" value="중복확인"/>
				<span class="red duple_txt hide">사용가능</span>
<?php }?>
			</td>
		</tr>
		<tr>
			<th>라벨프린터<span class="required_chk"/></th>
			<td>
<?php if($TPL_VAR["config_epost"]["status"]=='9'){?>
...
<?php }?>
		...
		<div class="personInfo" style="padding-top:10px;font-size:12px;">
			※ 사업자정보 변경은 <a href="/admin/setting/basic" target="_blank"><span class="highlight-link hand" style="vertical-align:text-top !important;">설정>02.일반정보>사업자정보</span></a> 에서 수정하세요.
		</div>
		-->
		...
		<div class="personInfo" style="padding-top:20px;">
			...
			<tr>
				<td>우체국 아이디, 우체국 비밀번호</td>
				<td>우체국 택배 연동 승인을 위한 처리</td>
				<td>개인정보 수집 및 이용 목적 달성 후 즉시 파기</td>
			</tr>
			<tr>
				<td>
			...
			<div class="chk_area" style="padding-top:10px;">
				<div class="resp_radio">
...
<?php }?>
		<input type="button" class="resp_btn v3 size_XL" value="닫기" onclick="closeDialogEvent(this);" />
	</div>
	</form>
</div>
<!-- 우체국택배 연동설정 팝업창 :: END -->
...
<?php }?>
	...
	<tr>
		...
		<th><span class="red">*</span> 쇼핑몰명</th>
		<td>
	...
	<tr>
		<th><span class="red">*</span> 발송지전화1</th>
		<td>
	...
	<tr>
		<th><span class="red">*</span> 사업자 번호</th>
		<td>
...
<?php }}?>
	...
	<tr>
		<th><span class="red">*</span> 택배사 계약코드</th>
		<td>
...
<?php }else{?>
		<th>
			<span class="btn small gray"><input class="del_price" type="button" value="-"></span>
		</th>
<?php }?>
		<td colspan="3">
...
<div id="goodsflow_use_area" class="hide">
	...
	<tr>
		<td colspan="2" height="50px">▣ 굿스플로 자동화 서비스 이용유무 선택</td>
	</tr>
	<tr>
		<td width="5%" height="30px">
			<input type="radio" name="goodsflow_use" id="use0" value="0" <?php if($TPL_VAR["config_system"]["goodsflow_use"]==='0'){?>checked<?php }?> />
		</td>
		<td><label for="use0">본사만 이용 가능(어떤 입점사도 이용할 수 없습니다.)</label></td>
	</tr>
	<tr>
		<td width="5%" height="30px">
			<input type="radio" name="goodsflow_use" id="use1" value="1" <?php if($TPL_VAR["config_system"]["goodsflow_use"]==='1'){?>checked<?php }?> />
		</td>
		<td>
			<label for="use1">본사와 입점사 모두 이용 가능(서비스 신청하고 충전 잔여 건수 있을때 이용 가능)</label>
			<br/><span class="desc">※ 개별 입점사별로 입점사 굿스플로 설정에서 이용 가능 유무 설정할 수 있습니다.</span>
		</td>
	</tr>
	</table>
	<div style="text-align:center; padding-top:30px;">
		<span class="btn large cyanblue"><input type="button" id="gf_setting_save" value="저장하기" /></span>
	</div>
</div>
<!-- 굿스플로 결제 -->
...
<div id="goodsflow_log_area" class="hide">
	<iframe name="goodsflow_log" id="goodsflow_log" src="./goodsflow_log" width="100%" height="490" frameborder="0"></iframe>
	<div class="footer">
...
<div id="desc_popup_area" class="hide">
	...
	<tr>
		<td>출력 방법</td>
		<td>우체국택배(자동) 출고 후<br/>우체국 택배 관리자에서 출력</td>
		<td>롯데택배(자동) 출고 후<br/>출력 가능</td>
		<td>
			굿스플로 자동화로 출고<br/>
