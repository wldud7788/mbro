<?php /* Template_ 2.2.6 2022/05/17 12:29:27 /www/music_brother_firstmall_kr/selleradmin/skin/default/refund/catalog.html 000009412 */ ?>
	...
	<div class="search-form-container-new search_container">
		<table class="table_search">
			<tr>
				<th>검색어</th>
				<td>
					<input type="text" name="keyword" value="<?php echo $_GET["keyword"]?>" title="환불번호, 아이디, 회원명, 주문자명, 수령자명, 상품명(매입상품명,사은품명), 상품코드" size="100"/>
	</div>
	</td>
	</tr>
	</table>
...
<?php }?>
						...
						<tr>
							...
							<span class="resp_checkbox">
								...
								<label class="search_label"><input type="checkbox" name="refund_status[]" value="complete" <?php if($_GET["refund_status"]&&in_array('complete',$_GET["refund_status"])){?>checked<?php }?> row_check_all />환불완료</label>
							</span>
...
<?php if($TPL_VAR["npay_use"]){?>
							<th>Npay 취소요청</th>
							<td no=1>
								<label class="resp_checkbox"><input type="checkbox" name="search_npay_order_cancel" value=1 <?php if($_GET["search_npay_order_cancel"]){?>checked<?php }?>> 조회</label>
							</td>
<?php }?>
						</tr>
					</table>
