<?php /* Template_ 2.2.6 2022/09/15 17:42:15 /www/music_brother_firstmall_kr/selleradmin/skin/default/popup/zipcode_zone.html 000029471 */ ?>
		...
		for(var i=0;i<data.length;i++){
			...
			if(data[i][address_type] == ''){ // 중간이 빈 값에 대한 예외처리
				...
				if(address_type == 'SIGUNGU')
					...
					tmpSel = $("select[name='SIGUNGU']").val();
				...
				if(address_type == 'SIDO') diff_str = data[i][address_type];
...
function resetchildAddress(address_type){
	...
	if(address_type == 'SIDO'){
		...
		DONG				= '';
		parent_address_str	= SIDO;
		if	(SIGUNGU)	parent_address_str	+= ' ' + SIGUNGU;
		$("select[name='DONG']").empty().data('options');
		$("select[name='STREET']").empty().data('options');
	}else if(address_type == 'STREET'){
		parent_address_str	= SIDO;
		if	(SIGUNGU)	parent_address_str	+= ' ' + SIGUNGU;
		if	(DONG)		parent_address_str	+= ' ' + DONG;
		$("select[name='STREET']").empty().data('options');
	}
...
function select_address(){
	...
	if(SIGUNGU)	address_arr.push(SIGUNGU);
	if(DONG)	address_arr.push(DONG);
...
function add_address(sel_address, add_type, num){
	...
	if(sel_address[0])	sel_address_txt += sel_address[0];
	if(sel_address[1])	sel_address_txt += ' ' + sel_address[1];
	if(sel_address[2])	sel_address_txt += ' ' + sel_address[2];
	...
	if(add_type == 'add'){ // set일때는 disabled 처리 X
		...
		if	(nowPos == 3 || nowPos == 4){
			...
			$("select.address_select").eq(nowPos).find("option[value='" + sel_address[nowPos] + "']").attr('disabled', true);
			$("select.address_select").eq(nowPos).focus();
			if(nowPos == 2){
				$("select[name='STREET']").empty().data('options');
			}else{
				$("select.address_select").eq(nowPos+1).empty().data('options');
			}
		}
	}else{
	...
	var td_html = '<tr><td><label class="resp_checkbox"><input type="checkbox" class="chk" /></label></td><td>' + num + '</td><td>&nbsp;&nbsp;&nbsp;' + sel_address_txt + '<span class="hide_input">' + input_hide1 + input_hide2 + input_hide3 + input_hide4 + '</span></td></tr>';
...
function excel_upload()
{
	if	(!$("input#zone_excel_file").val()){
		...
		$.ajax({
			...
			processData: false,
			contentType: false,
			url: '../../selleradmin/setting/shipping_zone_upload',
			data: formData, 
			success: function (res){
		...
		$.ajax({
			...
			success : function(args) {
				if(args == 'ERROR'){
					...
					window.location.href = '/selleradmin/excel_spout/file_download?down_url='+ args;
				}
			}
		})
	});
...
<div class="content">
		...
		<table class="table_basic thl mt10">		
			<tr>
				...
				<td>
					...
					<button class="resp_btn active" type="button" onclick="submit_zone();">&nbsp;&nbsp;&nbsp;적용&nbsp;&nbsp;&nbsp;</button></span>
				</td>
			</tr>	
			<tr>				
				<td colspan="2">
					...
					<div style="padding-right:15px;float:left;">
						<select name="SIDO" class="address_select" address_type="SIGUNGU" style="border: 1px solid #cccccc; width:165px; height:350px" size="22">
						</select>
					</div>
					<!-- 시/도 선택 :: END -->
					...
					<!-- 시/구/군 선택 :: START -->
					<div style="padding-right:15px;float:left;">
						<select name="SIGUNGU" class="address_select" address_type="DONG" style="border: 1px solid #cccccc; width:165px; height:350px" size="22">
						</select>
					</div>
					<!-- 시/구/군 선택 :: END -->
					...
					<!-- 읍/면/동 선택 :: START -->
					<div style="padding-right:15px;float:left;">
						<select name="DONG" class="address_select" address_type="STREET" style="border: 1px solid #cccccc; width:165px; height:350px" size="22">
						</select>
					</div>
					<!-- 읍/면/동 선택 :: END -->
					...
					<div style="float:left;">
						<select name="STREET" style="border: 1px solid #cccccc; width:270px; height:350px" size="22" multiple>
						</select>
					</div>
					<!-- 도로명주소 선택 :: END -->
				</div>
	...
	<div id="zone_excel" class="hide" style="padding-bottom:20px;">
		<table class="table_basic thl mt10">		
			<tr>
				<th>관리명</th>
	...
	<div class="table_row_frame mt30">	
		<div class="dvs_top">	
			...
			<div class="dvs_right">	
				<button type="button" id="excel_down" class="resp_btn"><img src="/admin/skin/default/images/common/btn_img_ex.gif" align="absmiddle" /> 다운로드</span></button>
			</div>
		</div>
		...
		<table id="select_address_tb" class="table_row_basic">
			<thead>
				<tr>
					...
					<th>번호</th>
					<th>
<?php if($_GET["nation"]=='korea'){?> 대한민국 <?php }else{?> 해외국가 <?php }?>
					</th>
				</tr>
			</thead>
