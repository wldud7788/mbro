<?php /* Template_ 2.2.6 2022/05/17 12:29:06 /www/music_brother_firstmall_kr/selleradmin/skin/default/export/catalog_ajax.html 000025552 */ 
...
<?php if($TPL_V1["end_step"]=='45'){?>
						<span class="btn small red" onclick="export_proc('45','<?php echo $TPL_V1["end_step"]?>');"><button type="button">출고상태변경</button></span>
<?php }elseif($TPL_V1["end_step"]=='55'){?>
						<span class="btn small red" onclick="export_proc('55','<?php echo $TPL_V1["end_step"]?>');"><button type="button">출고상태변경</button></span>
<?php }elseif($TPL_V1["end_step"]=='65'){?>
						<span class="btn small red" onclick="export_proc('65','<?php echo $TPL_V1["end_step"]?>');"><button type="button">출고상태변경</button></span>
<?php }?>
						<img src="/admin/skin/default/images/common/btn_print_m_rls.gif" name="goods_print" id="<?php echo $TPL_V1["end_step"]?>" class="hand" align="absmiddle" />
<?php if($TPL_VAR["status_invoice_cnt"][$TPL_V1["end_step"]]){?>
						<img src="/admin/skin/default/images/common/btn_print_m_ivc.gif" name="invoice_print" id="<?php echo $TPL_V1["end_step"]?>" class="hand" align="absmiddle" />
<?php }?>
						<span class="hand reverse_export" id="<?php echo $TPL_V1["end_step"]?>">
<?php if($TPL_V1["end_step"]=='45'){?>
...
<?php }?>
				...
				<div class="list-end-total-amount">
					...
					&nbsp;&nbsp;&nbsp;
					<?php echo get_currency_price($TPL_V1["end_step_settleprice"], 2,'','<span class="fx14 order-step-color-'.$TPL_V1["end_step"].'">_str_price_</span>')?>
...
<?php }?>
					...
					<ul class="left-btns clearbox">
						<li>
							...
							<option value="select">전체선택</option>
							<option value="not-select">선택안함</option>
							<option value="important">별표선택(노)</option>
							<option value="soldout">별표선택(빨)</option>
							<option value="not-important">별표없음</option>
							</select>
						</li>
						<li>
<?php if($TPL_VAR["status_invoice_cnt"][$TPL_V1["status"]]){?>
							<img src="/admin/skin/default/images/common/btn_print_m_ivc.gif" name="invoice_print" id="<?php echo $TPL_V1["status"]?>" class="hand" align="absmiddle" />
<?php }?>

<?php if($TPL_V1["status"]=='45'){?>
							<span class="btn small red" onclick="export_proc('45','<?php echo $TPL_V1["status"]?>');"><button type="button">출고상태변경</button></span>
<?php }elseif($TPL_V1["status"]=='55'){?>
							<span class="btn small red" onclick="export_proc('55','<?php echo $TPL_V1["status"]?>');"><button type="button">출고상태변경</button></span>
<?php }elseif($TPL_V1["status"]=='65'){?>
							<span class="btn small red" onclick="export_proc('65','<?php echo $TPL_V1["status"]?>');"><button type="button">출고상태변경</button></span>
<?php }?>

							<span class="hand reverse_export" id="<?php echo $TPL_V1["status"]?>">
<?php if($TPL_V1["status"]=='45'){?>
							<span class="helpicon" title="출고 준비된 상품을 상품준비로 되돌릴 수 있습니다."></span> '상품준비'로 되돌리기 <img src="/admin/skin/default/images/common/icon_arrow_back.gif" align="absmiddle" />
<?php }elseif($TPL_V1["status"]=='55'){?>
							<span class="helpicon" title="출고 완료된 상품을 출고준비로 되돌릴 수 있습니다.<br/>이 때 출고완료 시 차감된 재고 수량이 환원됩니다."></span> '출고준비'로 되돌리기 <img src="/admin/skin/default/images/common/icon_arrow_back.gif" align="absmiddle" />
...
<?php }?>
					...
					<ul class="right-btns clearbox">
						<li>
							...
							<span class="btn small"><button name="download_list">항목설정<span class="arrowright"></span></button></span>
							<span class="btn small"><button name="excel_down" status="<?php echo $TPL_V1["status"]?>"><img src="/admin/skin/default/images/common/btn_img_ex.gif" align="absmiddle" /> 일괄다운로드</button></span>
							<span class="btn small"><button name="excel_upload" status="<?php echo $TPL_V1["status"]?>"><img src="/admin/skin/default/images/common/btn_img_ex.gif" align="absmiddle" /> 일괄업로드(출고)</button></span>
						</li>
					</ul>
...
<?php }?>
		...
		<tr class="list-row step<?php echo $TPL_V1["status"]?>">
			<td align="center">
				<input type="checkbox" name="export_code[]" class="export_code_<?php echo $TPL_V1["status"]?>" value="<?php echo $TPL_V1["export_code"]?>" order_seq="<?php echo $TPL_V1["order_seq"]?>" goods_kind="<?php echo $TPL_V1["goods_kind"]?>" able_export="<?php echo $TPL_V1["able_export"]?>" />
			</td>
			<td align="center">
...
<?php if($TPL_V1["mbinfo_rute"]=='facebook'){?>
							<div>
								(<?php echo $TPL_V1["mbinfo_email"]?></span>/<span class="blue"><?php echo $TPL_V1["group_name"]?></span>)
							</div>
<?php }else{?>
							<div>
...
<?php }?>
			...
			<td align="center">
				<input type="text" name="export_date[<?php echo $TPL_V1["export_code"]?>]" class="line datepicker waybill_number tahoma" size="8" value="<?php echo $TPL_V1["export_date"]?>" style="font-size:11px;" />
			</td>
<?php if($TPL_V1["goods_kind"]=='coupon'){?>
...
<?php }else{?>
			<td align="left" class="pdl10 shipping_info_<?php echo $TPL_V1["export_code"]?>">
				<!-- 배송 정보 :: START -->
...
<?php if($TPL_V1["mdelivery"]=="대신택배"){?>
								<span class="btn small cyanblue"><button type="button" onclick="window.open('<?php echo $TPL_V1["tracking_url"]?>')" target="_blank" class="hand">조회</button></span>
<?php }else{?>
...
<?php }elseif($TPL_V1["status"]=='65'){?>
						<span class="btn small red" onclick="export_proc('65','<?php echo $TPL_V1["status"]?>');"><button type="button">출고상태변경</button></span>
<?php }?>
						<img src="/admin/skin/default/images/common/btn_print_m_rls.gif" name="goods_print" id="<?php echo $TPL_V1["status"]?>" class="hand" align="absmiddle" />
<?php if($TPL_VAR["status_invoice_cnt"][$TPL_V1["status"]]){?>
						<img src="/admin/skin/default/images/common/btn_print_m_ivc.gif" name="invoice_print" id="<?php echo $TPL_V1["status"]?>" class="hand" align="absmiddle" />
<?php }?>
						<span class="hand reverse_export" id="<?php echo $TPL_V1["status"]?>">
...
<?php }?>
				...
				<div class="list-end-total-amount">
					...
					&nbsp;&nbsp;&nbsp;
					<?php echo get_currency_price($TPL_V1["last_step_settleprice"], 2,'','<span class="fx14 order-step-color-'.$TPL_V1["step"].'">_str_price_</span>')?>
...
<?php if($TPL_VAR["record"]){?>
	<tr class="list-row pageoverflow">
		<td colspan="11" align="center" ><span class="btn large"><button type="button" name="order_admin_person"  onclick="get_catalog_ajax();">더 보기 <span class="arrowright"></span></button></span></td>
	</tr>
<?php }?>
<?php }?>
