<?php /* Template_ 2.2.6 2022/05/17 12:29:07 /www/music_brother_firstmall_kr/selleradmin/skin/default/export/view.html 000042356 */ 
...
<!-- ######################## 16.12.16 gcs yjy : 검색조건 유지되도록 -->
			...
			<li <?php if($TPL_VAR["data_export"]["able_export"]){?>class="hand reverse_export"<?php }else{?> class="hand" onclick="alert('본사에서 배송한 출고 입니다.');"<?php }?>>
			&nbsp;
...
<?php }elseif($TPL_VAR["data_export"]["status"]=='55'&&!$TPL_VAR["coupon_cnt"]&&(!$TPL_VAR["npay_use"]||!$TPL_VAR["data_export"]["npay_order_id"])){?>
...
<?php }?>
<?php if($TPL_VAR["data_export"]["status"]< 75&&$TPL_VAR["data_export"]["status"]>= 45&&(!$TPL_VAR["npay_use"]||!$TPL_VAR["data_export"]["npay_order_id"])){?>
			<li><span class="btn large black"><button type='button' name="export_modify" id="export_modify">출고정보저장</button></span></li>
<?php }?>
<?php if($TPL_VAR["data_export"]["status"]>= 45&&$TPL_VAR["data_export"]["status"]< 75&&(!$TPL_VAR["npay_use"]||!$TPL_VAR["data_export"]["npay_order_id"])){?>
...
<?php if(!$TPL_VAR["orders"]["npay_order_id"]){?>
<?php if($TPL_VAR["data_export"]["buyconfirmInfo"]['btn_buyconfirm']){?>
			<span class="btn small red"><button  type="button" <?php if($TPL_VAR["data_export"]["able_export"]){?>name="buy_confirm" class="buy_confirm"<?php }else{?> onclick="alert('본사에서 배송한 출고 입니다.');"<?php }?>>구매확정</button></span>
<?php }?><?php }?>
