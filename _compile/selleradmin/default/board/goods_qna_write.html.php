<?php /* Template_ 2.2.6 2022/05/17 12:28:52 /www/music_brother_firstmall_kr/selleradmin/skin/default/board/goods_qna_write.html 000020484 */ 
...
<table class="table_basic thl">
	<tbody>
		<tr>
			<th>작성자</th>
			<td<?php if($TPL_VAR["emoneylay"]||$TPL_VAR["emoneyviewlay"]){?><?php }else{?> colspan="3"<?php }?>><?php echo $TPL_VAR["name"]?> (IP:<?php echo $TPL_VAR["ip"]?>)
<?php if($TPL_VAR["mseq"]=='-1'){?>
...
<?php if($TPL_VAR["mseq"]){?>
					...
					<input type="hidden" name="name" id="name" value="<?php echo $TPL_VAR["real_name"]?>"  title="작성자를 입력해 주세요" class="required line" />
<?php }else{?>
					<?php echo $TPL_VAR["real_name"]?>

					<input type="hidden" name="name" id="name" value="<?php echo $TPL_VAR["real_name"]?>"  title="작성자를 입력해 주세요" class="required line" />
<?php }?>
<?php }?>
			(IP:<?php echo $TPL_VAR["ip"]?>)</td>
			...
			<td>
				...
				<div class="funcSmsSend<?php if($TPL_VAR["manager"]["sms_reply_user_yn"]=="Y"&&$TPL_VAR["rsms"]=='Y'&&$TPL_VAR["tel1"]){?><?php }else{?> hide<?php }?>">
					<p>
						...
						<span class="gray fx11">잔여 SMS:<?php echo ($TPL_VAR["count"])?>건</span>
					</p>
					<p>
						<span class="gray fx11"><?php if($TPL_VAR["manager"]["sms_reply_user_yn"]!='Y'){?>게시판 설정 &gt; <a href="/admin/board/manager_write?id=<?php echo $_GET["id"]?>" target="_blank">SMS발송</a>을 설정해주세요.<?php }?></span>
					</p>
				</div>
			</td>
		</tr>
...
<?php $this->print_("noticehidden",$TPL_SCP,1);?>
		...
		<tr>
			...
			<td>
				<button type="button" name="issueGoodsButton" id="issueGoodsButton" title="<?php echo $TPL_VAR["manager"]["name"]?>" class="resp_btn active">상품 선택</button>
			</td>
		</tr>
		<tr>
			<td>
