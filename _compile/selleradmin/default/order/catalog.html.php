<?php /* Template_ 2.2.6 2022/09/15 17:42:15 /www/music_brother_firstmall_kr/selleradmin/skin/default/order/catalog.html 000011685 */ ?>
...
<link rel="stylesheet" type="text/css" href="/selleradmin/skin/default/css/layer_stock.css?v=<?php echo date('Ymd')?>" />
<style>
...
<?php }?>
		...
		<div class="page-title">
			<h2><?php if($TPL_VAR["sc"]["shipping_provider_seq"]== 1){?>위탁배송 주문상품<?php }else{?>입점사배송 주문상품<?php }?></h2>
		</div>
...
<?php }?>
			<col width="50" />
			<col width="100" />
<?php if($TPL_VAR["detailmode"]!='trust'){?><col width="40" /><?php }?>
			<col width="250" />
			<col />
			<col width="65" />
<?php if($TPL_VAR["detailmode"]!='trust'){?><col width="60" /><?php }?>
			<col width="140" />
			<col width="120" />
		</colgroup>
		<thead class="lth">
...
<?php }?>
			<th>번호</th>
			<th>주문일시</th>
<?php if($TPL_VAR["detailmode"]!='trust'){?><th>환경</th><?php }?>
			<th>주문번호</th>
			<th>주문상품</th>
			<th>수(종)</th>
<?php if($TPL_VAR["detailmode"]!='trust'){?>
...
<div id="print_setting_dialog" class="hide" style="line-height:20px;"><?php $this->print_("print_setting",$TPL_SCP,1);?></div>
