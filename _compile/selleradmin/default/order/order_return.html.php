<?php /* Template_ 2.2.6 2021/08/25 16:21:11 /www/music_brother_firstmall_kr/selleradmin/skin/default/order/order_return.html 000035028 */ 
				...
				$("#order_return_container input[name='chk_suboption_seq[]'][item_option_seq='"+chk_option_seq+"']").each(function(e){
					...
					if($(this).val() != ''){
						...
						if(sub_disabled != 'disabled'){
							...
							row2.find("select[name='chk_ea[]']").attr("selected",true).parent().change();
						}
					}
				});
			}
<?php }?>
			...
			if(chk_suboption_seq != ''){
				if(sub_disabled_non > 0 ){
					if($(this).is(":checked") == false){
						...
						opt.find("select[name='chk_ea[]'] option:last-child").attr("selected",false).parent().change();
		...
		if($("#order_return_container input[name='chk_seq[]']:checked").length > 0){
			...
			$("#order_return_container input[name='chk_seq[]']:checked").not(":disabled").each(function (idx){
				var shipObj = $(this).closest('tr').find("input[name='pay_shiping_cost[]']");
...
<div id="order_return_container">
	...
	<form name="refundForm" method="post" action="/selleradmin/order_process/order_return" target="actionFrame" onsubmit="loadingStart();">
	...
	<div class="shipping_group">
		...
		<tr>
			<td class="cell left" colspan="7">
				↓ 출고번호 : <?php echo $TPL_V1["export"]["export_code"]?> &nbsp;&nbsp; 배송지 : <?php echo $TPL_VAR["orders"]["recipient_user_name"]?> /  <?php if($TPL_VAR["orders"]["recipient_address_type"]=="street"){?> <?php echo $TPL_VAR["orders"]["recipient_address_street"]?><?php }else{?> <?php echo $TPL_VAR["orders"]["recipient_address"]?> <?php }?> <?php echo $TPL_VAR["orders"]["recipient_address_detail"]?>
...
<?php if($TPL_I2== 0){?>
					<input type="hidden" name="pay_shiping_cost[]" shipping_seq="<?php echo $TPL_V2["shipping_seq"]?>" value="<?php if($TPL_V2["swap_refund_shiping_cost"]){?><?php echo $TPL_V2["swap_refund_shiping_cost"]?><?php }else{?><?php echo $TPL_V2["refund_shiping_cost"]?><?php }?>" />
<?php }?>
...
<?php if($TPL_V2["opt_type"]=='opt'){?>
						<img src="<?php echo $TPL_V2["image"]?>" align="absmiddle" hspace="5" width="45" height="45" style="border:1px solid #ddd;" onerror="this.src='/selleradmin/skin/default/order/images/common/noimage_list.gif'" />
<?php }else{?>
...
<?php if($TPL_V3["type"]=='file'){?>
								<a href="/selleradmin/order_process/filedown?file=<?php echo $TPL_V3["value"]?>" target="actionFrame" style="color:#848484;"><?php echo $TPL_V3["value"]?></a>
...
<?php if((!$TPL_VAR["npay_use"]||$TPL_VAR["orders"]["pg"]!='npay')&&$_GET["mode"]!='exchange'){?>
	<div>● 환불 방법</div>
	<div>
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="list_table_style">
	<thead>
