<?php /* Template_ 2.2.6 2022/05/17 12:29:09 /www/music_brother_firstmall_kr/selleradmin/skin/default/goods/excel_upload.html 000016466 */ 
...
<div id="page-title-bar-area">
	<div id="page-title-bar">
		...
		<div class="page-title">
			<h2><span class="icon-goods-kind-goods"></span>일괄 실물상품등록/수정</h2>
		</div>

		<!-- 좌측 버튼 -->
		<ul class="page-buttons-left">
			<li><span class="btn large"><button type="button" onclick="location.href='catalog';">상품리스트</button></span></li>
		</ul>
...
<div class="upload-form">
	...
	<div class="item-title">.xlsx 형식으로 저장된 파일 업로드 → 실물상품 일괄 등록/수정 </div>
	<div class="seller-goods-status-lay">
		...
		<tr>
			...
			<th>가격대체(3개 열)</th>
			<th>추가옵션(<?php echo count($TPL_VAR["suboptions"])?>개 열)</th>
			<th>기타 모든 정보</th>
		</tr>
		<tr>
	...
	<div class="notice" style="color:#000;">
		<div>실행타입 Type1 : 상품수정 시 상품명(1개 열), 옵션(<?php echo count($TPL_VAR["options"])?>개 열), 
							가격대체(3개 열), 추가옵션(<?php echo count($TPL_VAR["suboptions"])?>개 열)의 
							총 <?php echo (count($TPL_VAR["options"])+count($TPL_VAR["suboptions"])+ 4)?>개 열의 정보가 
							업데이트되지 않습니다.</div>
	...
	<div class="notice" style="color:#000;">
		<div>실행타입 Type2 : 상품수정 시 상품명(1개 열), 옵션(<?php echo count($TPL_VAR["options"])?>개 열), 
							가격대체(3개 열), 추가옵션(<?php echo count($TPL_VAR["suboptions"])?>개 열)의 
							총 <?php echo (count($TPL_VAR["options"])+count($TPL_VAR["suboptions"])+ 4)?>개 열의 정보 및 
	...
	<div class="notice">
		[주의] '(구) 실물 다운로드'로 받은 파일은 반드시 <b>'(구) 일괄 실물상품등록/수정'</b>으로 업로드 하세요!
		<span class="btn small cyanblue"><button type="button" onclick="open_old_excel_upload();">(구) 일괄 실물상품등록/수정</button></span>
	</div>
	</form>
</div>
...
<div class="upload-notice">
	<div class="item-title red">
		...
		<span class="btn small cyanblue"><button type="button" onclick="download_sample();">설명용 샘플파일 다운로드(실물-Seller)</button></span>
	</div>
	...
	<tr>
		<td>
			...
			<div>3. 업로드 파일로 보기만 가능한 열 <span class="red">3</span>개</div>
			<div>- 보기 열 : 승인여부, 수수료, 추가옵션수수료</div>
			<br style="line-height:20px;" />
			...
			<div>5. 업로드 파일로 추가옵션 정보를 수정하고자 할 때 조건 (추가옵션 정보를 수정할 필요가 없으면 
			<?php echo count($TPL_VAR["suboptions"])?>개의 열을 모두 없애십시오)</div>
			<div>- 조건1) 아래의 <span class="red"><?php echo count($TPL_VAR["suboptions"])?></span>개 열이 SET로 반드시 있어야 함</div>
			...
			<div>- 조건2)</div>
			<div style="margin-left:13px;">추가옵션명의 행 수 = 추가옵션값의 행 수 = 추가옵션수수료의 행 수 = 추가옵션매입가의 행 수 = 추가옵션정가의 행 수 = 추가옵션할인가(판매가)의 행 수 = 추가옵션마일리지의 행 수</div>
			<br style="line-height:20px;" />

			<div>6. 업로드 파일로 추가입력옵션 정보를 수정하고자 할 때 조건 (추가입력옵션 정보를 수정할 필요가 없으면 <?php echo count($TPL_VAR["inputs"])?>개의 열을 모두 없애십시오)</div>
