<?php /* Template_ 2.2.6 2022/05/17 12:29:09 /www/music_brother_firstmall_kr/selleradmin/skin/default/goods/edit_goods_options.html 000074172 */ 
...
<script type="text/javascript">
	...
	var gl_skin_currency					= '<?php echo $TPL_VAR["config_system"]["skin_currency"]?>';
	var gl_basic_currency_symbol			= '<?php echo $TPL_VAR["config_system"]["basic_currency_symbol"]?>';
	var gl_basic_currency_symbol_position	= '<?php echo $TPL_VAR["config_system"]["gl_basic_currency_symbol_position"]?>';
...
<?php if($TPL_VAR["goods"]["reserve_policy"]=='shop'){?>
...
<?php }?>
	...
	function save_input_value(obj){
		...
		if	($(obj).attr('name') == 'price' || $(obj).attr('name') == 'consumer_price' || $(obj).attr('name') == 'commission_rate' || $(obj).attr('name') == 'reserve_rate' || $(obj).attr('name') == 'reserve_unit'){
			tmpOptCalculate(obj);
		}

		param['reserve']	= uncomma_float($(obj).closest('tr').find("input[name='reserve']").val());
	...
	function tmpOptCalculate(obj){
		...
		var commission_type	= (commission_type) ? commission_type : '<?php echo $TPL_VAR["default_charge"]["commission_type"]?>';
		...
		if(commission_cal != 'NONE'){
			var sale_price			= parseInt($(obj).closest("tr").find("input[name='price']").val(),10);
			var consumer_price		= parseInt($(obj).closest("tr").find("input[name='consumer_price']").val(),10);
			var commission_rate		= parseFloat($(obj).closest("tr").find("input[name='commission_rate']").val(),10);
		...
		if	(calulateReserve){
			...
			$(obj).closest("tr").find(".reserve-goods").html("("+reserve+")");
			$(obj).closest("tr").find(".reserve_view").html(reserve);
			$(obj).closest("tr").find("input[name='reserve']").val(reserve);
		}
	}
	...
	function tmpReserveCalculate(){
		...
		$("input[name='price']").each(function(){
			...
			$(this).closest("tr").find(".reserve_view").html(reserve);
			$(this).closest("tr").find("input[name='reserve']").val(reserve);
		});
	}
		...
		if	(target == 'tmp_policy'){
			...
			$("input[name='"+target+"'],select[name='"+target+"']").each(function(){
				...
				if	(target == 'option_view')
					$("select[name='option_view']:visible").val($("select[name='option_view_all']").val());
				else
					$(this).val(value);
...
//직접입력 > 색상
	...
	function doApplyOptInfomation() {
		...
		var newValue		= $('#tmpInfomation').val();
...
<?php if($TPL_VAR["package_yn"]!='y'&&!$_GET["socialcp_input_type"]){?>
					<th class="its-th-align center" width="80">
						...
						<span class="btn small"><button type="button" class="save_all" id="weight_all">▼</button></span>
					</th>
<?php }?>
<?php if($_GET["socialcp_input_type"]){?>
					<th class="its-th-align center">
						<input type="text" size="5" name="coupon_input_all" value="" />
						<span class="btn small"><button type="button" class="save_all" id="coupon_input_all">▼</button></span>
					</th>
<?php }?>
<?php if($_GET["package_yn"]!='y'){?>
					<th class="its-th-align center">
						<input type="text" size="5" name="stock_all" value="" />
						<span class="btn small"><button type="button" class="save_all" id="stock_all">▼</button></span>
					</th>
					<th class="its-th-align center">
						<input type="text" size="5" name="badstock_all" value="" />
						<span class="btn small"><button type="button" class="save_all" id="badstock_all">▼</button></span>
					</th>
					<th class="its-th-align center">
						<input type="text" size="5" name="safe_stock_all" value="" />
						<span class="btn small"><button type="button" class="save_all" id="safe_stock_all">▼</button></span>
					</th>
<?php }else{?>
					<th class="its-th-align center"></th>
<?php }?>
					<th class="its-th-align center <?php if($_GET["provider_seq"]== 1){?>hide<?php }?>"></th>
					<th class="its-th-align center">
						<input type="text" size="5" name="consumer_price_all" value="" />
						<span class="btn small"><button type="button" class="save_all" id="consumer_price_all">▼</button></span>
						→
						<input type="text" size="5" name="price_all" value="" />
						<span class="btn small"><button type="button" class="save_all" id="price_all">▼</button></span>
					</th>
					...
					<th class="its-th-align">
						...
						<span class="btn small"><button type="button" class="save_all" id="option_view_all">▼</button></span>
					</th>
					<th class="its-th-align">
...
<?php }else{?>
					<th class="its-th-align center" colspan="3">
						입점사
					</th>
<?php }?>
					<th class="its-th-align center" rowspan="2" width="100">
<?php if($TPL_VAR["default_charge"]["commission_type"]=='SACO'||$TPL_VAR["default_charge"]["commission_type"]==''){?>
						수수료
...
<?php if($_GET["package_yn"]!='y'){?>
					...
					<th class="its-th-align center">
						안전재고
						<a href="javascript:helperMessage('safeStock', '입점사');"><img src="/admin/skin/default/images/common/btn_help.gif"/></a>
					</th>
<?php }else{?>
...
<?php if($TPL_V1["package_goods_seq1"]){?>
										<a href="/goods/view?no=<?php echo $TPL_V1["package_goods_seq1"]?>" target="_blank"><span class="reg_package_goods_seq1"><?php if($TPL_V1["package_goods_seq1"]){?>[<?php echo $TPL_V1["package_goods_seq1"]?>]<?php }?></span>
<?php }?>
...
<?php if($TPL_V1["package_goods_seq2"]){?>
										<a href="/goods/view?no=<?php echo $TPL_V1["package_goods_seq2"]?>" target="_blank">
											<span class="reg_package_goods_seq2"><?php if($TPL_V1["package_goods_seq2"]){?>[<?php echo $TPL_V1["package_goods_seq2"]?>]<?php }?></span>
<?php }?>
...
<?php }?>
					<td class="its-td-align right pdr10">
						<span class="commission_rate"><?php echo $TPL_V1["commission_rate"]?></span>
						<input type="hidden" name="commission_rate" value="<?php echo $TPL_V1["commission_rate"]?>" />
<?php if($TPL_V1["commission_type"]=='SUPR'){?><?php echo $TPL_VAR["basic_currency"]?><?php }else{?>%<?php }?>
					</td>
					<td class="its-td-align right pdr10 pricetd">
						<input type="text" name="consumer_price" value="<?php echo $TPL_V1["consumer_price"]?>" size="7" style="color:#000;" onkeyup="key_input_value(event, this);" onfocus="ready_input_save(this);" onblur="save_input_value(this);" />
						→
						<input type="text" size="7" name="price" value="<?php echo $TPL_V1["price"]?>" onkeyup="key_input_value(event, this);" onfocus="ready_input_save(this);" onblur="save_input_value(this);" />
					</td>
					<td class="its-td-align right pdr10">
						<div class="reserve-shop-lay <?php if($TPL_VAR["goods"]["reserve_policy"]=='goods'){?>hide<?php }?>">
							...
							(<span class="reserve-shop"><?php echo get_currency_price($TPL_V1["shop_reserve"], 3)?></span>)
						</div>
						<div class="reserve-goods-lay <?php if($TPL_VAR["goods"]["reserve_policy"]=='shop'){?>hide<?php }?>">
<?php if($TPL_V1["reserve_unit"]=='percent'){?>
							<?php echo $TPL_V1["reserve_rate"]?>%
							(<span class="reserve-goods"><?php echo get_currency_price($TPL_V1["reserve"], 3)?></span>)
<?php }else{?>
							개당 <?php echo get_currency_price($TPL_V1["reserve"], 3)?>
...
<?php }?>
							...
							<div style="display:none;">
								...
								<select name="reserve_unit" onfocus="ready_input_save(this);" onchange="save_input_value(this);">
									...
									<option value="won" <?php if($TPL_V1["reserve_unit"]==$TPL_VAR["basic_currency"]){?>selected<?php }?>><?php echo $TPL_VAR["basic_currency"]?></option>
								</select>
								<span class="reserve_view"><?php echo get_currency_price($TPL_V1["reserve"])?></span>
								<input type="hidden" name="reserve" value="<?php echo get_currency_price($TPL_V1["reserve"])?>" />
							</div>
						</div>
					</td>
					<td class="its-td-align center">
					...
					<td  valign="top" class="left">
						<div class="addresslay"style="padding-top:10px;">
							<table width="100%" border="0" cellspacing="0" cellpadding="0" >
								<tr>
									<td valign="top" class="center" width="50">수수료</td>
...
<div class="center" style="padding:5px;">
	<span class="btn large"><button type="button" id="goodsoptiondirectmodifybtn">확인</button></span>
</div>
</form>
</div>
