<?php /* Template_ 2.2.6 2022/05/17 12:29:18 /www/music_brother_firstmall_kr/selleradmin/skin/default/goods/_set_suboptions_view.html 000017518 */ 
...
<?php }?>
		<th class="its-th-align center" rowspan="2">정산 금액</th>
...
<?php }else{?>
		...
		<th class="its-th-align center">안전재고 <span class="helpicon" title="입점사의 안전재고입니다.<br/>해당 상품의 재고수량이 안전재고 이하인 경우 자동 발주가 생성됩니다."></span></th>
<?php }?>
	</tr>
	</thead>
...
<?php }?>
		...
		<td class="its-td-align center subSettlementAmount"></td>
...
<?php if($TPL_VAR["reload"]=='y'){?>
	location.replace('?provider_seq=<?php echo $_GET["provider_seq"]?>&mode=view&tmp_seq=<?php echo $TPL_VAR["tmp_seq"]?>&tmp_policy=<?php echo $TPL_VAR["tmp_policy"]?>&goodsTax=<?php echo $TPL_VAR["goodsTax"]?>&goods_seq=<?php echo $TPL_VAR["goods_seq"]?>&socialcp_input_type=<?php echo $_GET["socialcp_input_type"]?>&islimit=<?php echo $TPL_VAR["islimit"]?>');
	socialcpinputtype();
<?php }else{?>
<?php if($TPL_VAR["suboptions"]){?>
