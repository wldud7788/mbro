<?php /* Template_ 2.2.6 2022/05/17 12:29:18 /www/music_brother_firstmall_kr/selleradmin/skin/default/goods/_set_suboptions_modify.html 000098480 */ 
...
<?php if($TPL_VAR["reload"]=='y'){?>
		location.replace('?provider_seq=<?php echo $_GET["provider_seq"]?>&tmp_seq=<?php echo $TPL_VAR["tmp_seq"]?>&tmp_policy=<?php echo $TPL_VAR["tmp_policy"]?>&goodsTax=<?php echo $TPL_VAR["goodsTax"]?>&goods_seq=<?php echo $TPL_VAR["goods_seq"]?>&socialcp_input_type=<?php echo $_GET["socialcp_input_type"]?>');
<?php }?>
...
<?php if($TPL_VAR["tmp_policy"]=='shop'){?>
...
<?php }?>
				...
				function calculrate_option(idx){
					...
					if	(price > 0){
<?php if($TPL_VAR["goodsTax"]=='exempt'){?>
						tax				= 0;
<?php }else{?>
...
<?php }?>
					...
					function setTmpSeq(){
						...
						opener.setSubOptionTmp('<?php echo $TPL_VAR["tmp_seq"]?>',tmp_frequently);
						self.close();
					}
...
<div style="width:99%;padding:10px;">
	<div id="suboptionLayer">
		<div class="top_title_memo">
			...
			<br/>
			<span class="red">★ 할인 이벤트 기간에 판매된 입점사의 이벤트 상품 수수료율</span> :
			프로모션/쿠폰 > <span class="bold orange">할인 이벤트</span>의 수수료 설정에 따라 결정됩니다. 본사와 입점사간 협의하여 결정해 주십시오.
		</div>
...
<?php }?>
		...
		<form name="listFrm" method="post" action="../goods_process/save_suboption_tmp" target="suboptionFrame">
			<input type="hidden" name="tmp_policy" value="<?php echo $TPL_VAR["tmp_policy"]?>" />
...
<?php if($TPL_VAR["default_charge"]["commission_type"]=='SACO'||$TPL_VAR["default_charge"]["commission_type"]==''){?>
						수수료
						<span class="helpicon" title="수수료 입력 시<br/>정산기준금액 = 판매가격 X (100-수수료)%"></span>
<?php }else{?>
						<span class="SUCO_title">
						공급가
						<span class="helpicon" title="1. 공급율 입력 시 정산기준금액 = 정상가격 X 공급율%<br/>2. 공급가 입력 시 정산기준금액 = 공급가격"></span>
					</span>
<?php }?>
					</th>
					<th class="its-th-align center" rowspan="2">
...
<?php }else{?>
					...
					<th class="its-th-align center">안전재고</th>
<?php }?>
				</tr>
				</thead>
				<tbody>
...
<?php }?>
									...
									<div>
										주문당
										<input type="text" name="tmp_package_unit_ea1[]" size="3" style="text-align:right;" value="<?php echo $TPL_V2["package_unit_ea1"]?>">
										발송
...
<?php }?>
					<td style="padding-right: 10px;" class="its-td-align right">
						<input style="text-align: right;" class="line onlynumber input-box-default-text" name="subCommissionRate[]" value="<?php if($TPL_V2["commission_rate"]){?><?php echo $TPL_V2["commission_rate"]?><?php }else{?><?php echo $TPL_VAR["default_charge"]["charge"]?><?php }?>" size="3" type="hidden">
						<input style="text-align: right;" class="line onlynumber input-box-default-text" name="subCommissionType[]" value="<?php if($TPL_V2["commission_type"]){?><?php echo $TPL_V2["commission_type"]?><?php }else{?><?php echo $TPL_VAR["default_charge"]["commission_type"]?><?php }?>" size="3" type="hidden">
						<input style="text-align: right;" class="line onlynumber input-box-default-text" disabled="disabled" value="<?php if($TPL_V2["commission_rate"]){?><?php echo $TPL_V2["commission_rate"]?><?php }else{?><?php echo $TPL_VAR["default_charge"]["charge"]?><?php }?>" size="3" type="text">
...
<?php if($TPL_V2["reserve_unit"]=='percent'){?>
						<?php echo number_format($TPL_V2["reserve_rate"])?>% (<?php echo get_currency_price($TPL_V2["reserve"], 3)?>)
<?php }else{?>
						<?php echo get_currency_price($TPL_V2["reserve"], 3)?>
