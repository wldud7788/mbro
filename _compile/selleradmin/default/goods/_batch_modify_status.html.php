<?php /* Template_ 2.2.6 2022/05/17 12:29:16 /www/music_brother_firstmall_kr/selleradmin/skin/default/goods/_batch_modify_status.html 000016783 */ 
$TPL_loop_1=empty($TPL_VAR["loop"])||!is_array($TPL_VAR["loop"])?0:count($TPL_VAR["loop"]);?>
<script type="text/javascript">
$(document).ready(function() {

	// 재고 정책 선택시
	$("select[name='all_runout_type']").bind("change",function(){ chk_runout_type(); });
	...
	$(".goods_status").bind("change",function(){
		if( $(this).parent().parent().find(".provider_status").val() == '0' && $(this).val() != 'unsold' ) {//미승인시
			openDialogAlert("해당 상품은 '미승인'처리되어 상품 상태는 '판매중지'가 됩니다.",500,150,function(){},'');
...
#dialog_confirm .ui-state-default { border: 1px solid #777777; background: #111111 url(/selleradmin/skin/default/goods/images/ui-bg_glass_40_111111_1x400.png) 50% 50% repeat-x; font-weight: normal; color: #e3e3e3; }
...
<table class="list-table-style" cellspacing="0">
	<colgroup>
		...
		<col width="" /><!--상품명-->
		<col width="90" /><!--승인/미승인-->
		<col width="120" /><!--노출/미노출-->
		<col width="130" /><!--상태-->
		<col width="160" /><!--재고에따른판매여부-->
		<col width="100" /><!--성인-->
...
<?php }?>
		...
		<th>
			<select name="all_cancel_type" class="all_cancel_type_value line" apply_target="cancel_type" apply_text="청약철회" style="width:65%;">
				<option value="0">청약철회불가</option>
				<option value="1">청약철회가능</option>
			</select><span class="btn small gray ml3"><button type="button" class="applyAllBtn"  apply_type='all_su_commission'>▼</button></span>
		</th>
	</tr>
	</thead>
</table>
<table class="list-table-style" cellspacing="0">
	...
	<tr>
		...
		<th style="border-top:0px;">승인/미승인</th>
		<th style="border-top:0px;">노출/미노출</th>
		<th style="border-top:0px;">상태</th>
		<th style="border-top:0px;">재고에따른판매여부</th>
		<th style="border-top:0px;">성인</th>
		<th style="border-top:0px;">해외구매대행</th>
		<th style="border-top:0px;">청약철회</th>
	</tr>
	</thead>
	<!-- 테이블 헤더 : 끝 -->
...
<?php if($TPL_V1["catename"]){?><div style="padding-top:5px;"><?php echo $TPL_V1["catename"]?></div><?php }?>
			</td>
			<!-- 승인/미승인 -->
			<td align="center">
<?php if($TPL_V1["provider_status"]=='1'){?>승인
<?php }elseif($TPL_V1["provider_status"]=='0'||$TPL_V1["provider_status"]==''){?>미승인<?php }?>
				<input type="hidden" name="provider_status[<?php echo $TPL_V1["goods_seq"]?>]" class="provider_status line" apply_type="provider_status_<?php echo $TPL_V1["goods_seq"]?>" value="<?php echo $TPL_V1["provider_status"]?>">
			</td>
			<!-- 노출/미노출 -->
			<td align="center">
				<div class="desc"><?php if($TPL_V1["display_terms"]=="AUTO"){?>자동<?php }else{?>수동<?php }?></div>
				<div class="mt3">
			...
			<td align="center">
				...
				<p class="goods_status_text desc"><?php echo $TPL_V1["goods_status_text"]?></p>
			</td>
...
<?php }?>
					...
					<div class="desc ml3">이상 일 때 판매</div>
				</div>
			</td>
			...
			<td align="center">
				<select name="cancel_type[<?php echo $TPL_V1["goods_seq"]?>]" class="cancel_type line" apply_type="cancel_type<?php echo $TPL_V1["goods_seq"]?>">
					<option value="0" <?php if($TPL_V1["cancel_type"]=="0"){?>selected<?php }?>>청약철회불가</option>
					<option value="1" <?php if($TPL_V1["cancel_type"]=="1"){?>selected<?php }?>>청약철회가능</option>
				</select>
			</td>
		</tr>
<?php }}?>
