<?php /* Template_ 2.2.6 2022/05/17 12:29:15 /www/music_brother_firstmall_kr/selleradmin/skin/default/goods/_batch_modify_ifgoods.html 000008257 */ 
	...
	<tbody class="ltb">
		<tr class="list-row" style="height:70px;">
			...
			<td>
				...
				<tr>
					<td><label><input type="checkbox" name="batch_goods_name_yn" value="1" /> 상품명을 </label></td>
					<td><input type='text' name="batch_goods_name" value="" size="40"> 변경합니다.</td>
				</tr>
				<tr>
					<td><label><input type="checkbox" name="batch_color_pick_yn" value="1" /> 검색 색상을</label></td>
					<td>
...
<?php if(is_array($TPL_R1=$TPL_VAR["arr_common"]['colorPickList'])&&!empty($TPL_R1)){foreach($TPL_R1 as $TPL_K1=>$TPL_V1){?>
							<label style="background-color:#<?php echo $TPL_K1?>;margin:1px;" class=" <?php if($TPL_V1== 1){?>active<?php }?>"><input type="checkbox" name="batch_color_pick[]" class="batch_color_pick" value="<?php echo $TPL_K1?>" <?php if($TPL_V1== 1){?>checked<?php }?> /></label>
<?php }}?>
							변경합니다.
						</div>
					</td>
				...
				<tr>
					<td><label><input type="checkbox" name="batch_keyword_yn" value="1" /> 추가 검색어를</label></td>
					<td><input type='text' name="batch_keyword" value="" size="40"> 변경합니다.</td>
				</tr>
				<tr>
					<td><label><input type="checkbox" name="batch_grade_sale_yn" value="1" /> 회원등급혜택을</label></td>
					<td>
						<select name="batch_grade_sale" class="batch_grade_sale line" >
...
<?php }}?>
				...
				<tr>
					<td><label><input type="checkbox" name="batch_tax_yn" value="1" /> 과세/비과세를</label></td>
					<td>
						<select name="batch_tax" class="batch_tax line">
				...
				<tr>
					<td><label><input type="checkbox" name="batch_summary_yn" value="1" /> 간략설명을</label></td>
					<td><input type='text' name="batch_summary" value="" size="40"> 변경합니다.</td>
				</tr>
			</table>
...
<?php if(in_array($TPL_K2,$TPL_V1["color_pick_list"])){?>
					<label style="background-color:#<?php echo $TPL_K2?>;margin:1px;cursor:none;"></label>
<?php }?>
<?php }}?>
				</div>
			</td>
